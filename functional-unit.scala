  ...
  with boom.ifu.HasBoomFrontendParameters
{
  ...
  if (isJmpUnit) {                      //如果是跳转指令，(即这个alu被例化为jmp_unit)
    ...
    val uop_pc = (block_pc | uop.pc_lob) - Mux(uop.edge_inst, 2.U, 0.U)  //正常的next_pc（前边或完是完整的pc）
  ...
  val op2_data_securitytag = Mux(uop.ctrl.op2_sel === OP2_IMM, uop_securitytag,
                 Mux(uop.ctrl.op2_sel === OP2_IMMC, uop_securitytag,  // CSR安全标签需单独处理
                 Mux(uop.ctrl.op2_sel === OP2_RS2 , io.req.bits.rs2_data_securitytag,
                 Mux(uop.ctrl.op2_sel === OP2_NEXT, uop_securitytag,  // NEXT操作安全标签统一处理
                                                    uop_securitytag))))

                            
  val data_check_unpass   =  ( ((~(uop_securitytag)) & ( io.req.bits.rs1_data_securitytag(1)))   |
                                                           ((~(uop_securitytag (1))) & (~(uop_securitytag(0)))&( io.req.bits.rs1_data_securitytag(0)))  |
                                                           ((~(uop_securitytag (0))) &  io.req.bits.rs1_data_securitytag(1))&( io.req.bits.rs1_data_securitytag(0))   ) &
                                                           ( ((~(uop_securitytag)) & ( io.req.bits.rs2_data_securitytag(1)))   |
                                                           ((~(uop_securitytag (1))) & (~(uop_securitytag(0)))&( io.req.bits.rs2_data_securitytag(0)))  |
                                                           ((~(uop_securitytag (0))) &  io.req.bits.rs2_data_securitytag(1))&( io.req.bits.rs2_data_securitytag(0))   )     

  dontTouch(data_check_unpass)
 ...
 //   io.resp.bits.data := reg_data
  ...
  r_data_securitytag(0) :=  Mux(io.req.bits.uop.is_sfb_br,  uop.uopc_securitytag , alu_out_securitytag)          
  ...
  for (i <- 1 until numStages) {                                                                                                         //流水线寄存器的段间连接
   ...
   r_data_securitytag(i) := r_data(i-1)
  ...
  io.bypass(0).bits.data_securitytag := Mux(io.req.bits.uop.is_sfb_br, uop.uopc_securitytag, alu_out_securitytag)       //同上的疑问
 ...
 */
class MemAddrCalcUnit(implicit p: Parameters)                                                   //访存地址计算单元
  extends PipelinedFunctionalUnit(
    numStages = 0,
    numBypassStages = 0,
  ...
  with freechips.rocketchip.rocket.constants.ScalarOpConstants
{
 ...
 //计算出来的地址最后生成为p_addr
  ...
  //io.resp.bits.sfence.bits.rs1_securitytag     //这是啥，好像是csr的东西，先不管
  ...
  //io.resp.bits.sfence.bits.addr_securitytag := io.req.bits.rs1_data_securitytag    //应该是csr的，暂时不管
