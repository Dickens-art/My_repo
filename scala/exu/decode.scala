...
class DecodeUnitIo(implicit p: Parameters) extends BoomBundle
{
  ...
  val enq_securitytag = Input(UInt(2.W))                      //输入待解码指令的securitytag
  
  val deq = new Bundle { val uop = Output(new MicroOp()) }

 val deq_securitytag = Output(UInt(2.W))                      //输出解码后指令的securitytag
 ...
 */
class DecodeUnit(implicit p: Parameters) extends BoomModule                               //修改后的单元见下文带securitytag后缀的
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  ...
  val cs = Wire(new CtrlSigs()).decode(inst, decode_table)             //控制信号
...
//     cs.div && !csr.io.status.isa('m'-'a') || TODO check for illegal div instructions
  ...
  val (xcpt_valid, xcpt_cause) = checkExceptions(List(                    //异常信息
  ...
  // repackage the immediate, and then pass the fewest number of bits around      重新封装imme以追求统一（最长20bit，出现在jal指令里）
  val di24_20 = Mux(cs.imm_sel === IS_B || cs.imm_sel === IS_S, inst(11,7), inst(24,20))    //如果是store指令或者branch指令，则取11_7位（offset字段），否则取24_20，可以参考IS_X定义
  uop.imm_packed := Cat(inst(31,25), di24_20, inst(19,12))                                             // 拼装为20位imme    

  // uop.imm_packed_sescuritytag := inst_securitytag                                                      //立即数的securitytag即该指令的securitytag                        
  ...
  io.deq.uop := uop                           //输出解码后的指令信息

 //deq_uop_securitytag := enq_uop_securitytag   //securitytag不需要解码
...
{
  val is_ret   = Bool()       //返回？
  val is_call  = Bool()      //call？
  val target   = UInt(vaddrBitsExtended.W)   //分支目标
  val cfi_type = UInt(CFI_SZ.W)                         //cfi（调用框架指令）类型（branch、jar、jalr）


  // Is this branch a short forwards jump?      //短跳转？
  val sfb_offset = Valid(UInt(log2Ceil(icBlockBytes).W))      //偏移量
  // Is this instruction allowed to be inside a sfb?      //
  val shadowable = Bool()
}

class BranchDecode(implicit p: Parameters) extends BoomModule          //分支解码
{
  val io = IO(new Bundle {
    val inst    = Input(UInt(32.W))                                            
    val pc      = Input(UInt(vaddrBitsExtended.W))

    val out = Output(new BranchDecodeSignals)         //输出指令信息（分支目标、跳转类型等）
  })

  val bpd_csignals =
    freechips.rocketchip.rocket.DecodeLogic(io.inst,
                  List[BitPat](N, N, N, N, X),       //用list表示分支信息
////                               is br?
////                               |  is jal?
////                               |  |  is jalr?
////                               |  |  |
 ...
 * Decode unit that takes in a single instruction and generates a MicroOp.
 */
class DecodeUnit_Security(implicit p: Parameters) extends BoomModule                //该单元师兄只加入了swt指令的解码，需要加入tag的传播
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  val io = IO(new DecodeUnitIo)
  ...
  val uop_securitytag = Wire(UInt(2.W))
  uop_securitytag := io.enq_securitytag


  var decode_table = XDecode_Security.table
  if (usingFPU) decode_table ++= FDecode.table
  if (usingFPU && usingFDivSqrt) decode_table ++= FDivSqrtDecode.table
  if (usingRoCC) decode_table ++= RoCCDecode.table
  decode_table ++= (if (xLen == 64) X64Decode.table else X32Decode.table)
  ...
  val cs = Wire(new CtrlSigs_Security()).decode(inst, decode_table)
...
//     cs.div && !csr.io.status.isa('m'-'a') || TODO check for illegal div instructions
  ...
  val (xcpt_valid, xcpt_cause) = checkExceptions(List(             //异常信息
    (io.interrupt && !io.enq.uop.is_sfb, io.interrupt_cause),  // Disallow interrupts while we are handling a SFB
    (uop.bp_debug_if,                    (CSR.debugTriggerCause).U),           //分支预测异常
    (uop.bp_xcpt_if,                     (Causes.breakpoint).U),                         
    (uop.xcpt_pf_if,                     (Causes.fetch_page_fault).U),              //页异常
    (uop.xcpt_ae_if,                     (Causes.fetch_access).U),                      //存取异常
    (id_illegal_insn,                    (Causes.illegal_instruction).U),         //非法指令

    // (uop.inst_check_unpass,                    (Causes.inst_tag_check_unpass).U) 
    ))            //指令tag检查不通过，0x18，在rocket的inst文件下
  ...
  //如果是store指令或者branch指令，则取11_7位（offset字段），否则取24_20，可以参考IS_X定义
  uop.imm_packed := Cat(inst(31,25), di24_20, inst(19,12))
  //拼装为20位imme

  uop.imm_packed_securitytag := uop_securitytag  //imm的tag即该指令的tag
  ...
  io.deq_securitytag := uop_securitytag                      //tag不需要改变
 ...
 * Decoded control signals
 */
class CtrlSigs_Security extends Bundle
{
  val legal           = Bool()
  val fp_val          = Bool()
  val fp_single       = Bool()
  ...
  def decode(inst: UInt, table: Iterable[(BitPat, List[BitPat])]) = {
    val decoder = freechips.rocketchip.rocket.DecodeLogic(inst, XDecode_Security.decode_default, table)
    val sigs =
      Seq(legal, fp_val, fp_single, uopc, iq_type, fu_code, dst_type, rs1_type,
          rs2_type, frs3_en, imm_sel, uses_ldq, uses_stq, is_amo,
          is_fence, is_fencei, mem_cmd, wakeup_delay, bypassable,
 ...
 * Overall Decode constants
 */
object XDecode_Security extends DecodeConstants
{
           //                                                                  frs3_en                        wakeup_delay
           //     is val inst?                                                 |  imm sel                     |    bypassable (aka, known/fixed latency)
           //     |  is fp inst?                                               |  |     uses_ldq              |    |  is_br
  ...
  //SWT指令
  CUSTOM0_RS1 -> List(Y, N, X, uopSTA  , IQT_MEM, FU_MEM , RT_X  , RT_FIX, RT_X  , N, IS_S, N, Y, N, N, N, M_XWR, 0.U, N, N, N, N, N, CSR.N),
