...
class FpPipeline(implicit p: Parameters) extends BoomModule with tile.HasFPUParameters
{
  ...
  val io = IO(new Bundle {
    ...
    val dis_uops_securitytag   =  Input(Vec(dispatchWidth,UInt(2.W) ))
    ...
    val debug_tsrc_reg    = Input(UInt(width=xLen.W))
    val debug_wb_wdata   = Output(Vec(numWakeupPorts, UInt((fLen+1).W)))

   //val debug_wb_wdata_securitytag   = Output(Vec(numWakeupPorts, UInt(2.W)))        //tag
  ...
  val fregfile       = Module(new RegisterFileSynthesizable(numFpPhysRegs,        //浮点寄存器
  ...
  val iss_uops_securitytag = Wire(Vec(exe_units.numFrfReaders, UInt(2.W)))    //tag

  issue_unit.io.tsc_reg := io.debug_tsrc_reg
  ...
  for (w <- 0 until dispatchWidth) {
    ...
    issue_unit.io.dis_uops_securitytag(w) <> io.dis_uops_securitytag(w)
  ...
  for (i <- 0 until issue_unit.issueWidth) {
    ...
    iss_uops_securitytag(i) := issue_unit.io.iss_uops_securitytag(i)
  ...
  fregister_read.io.prf_read_ports map { port => port.data_securitytag := 0.U(2.W) }

  fregister_read.io.iss_valids <> iss_valids
  fregister_read.io.iss_uops := iss_uops

  fregister_read.io.iss_uops_securitytag := iss_uops_securitytag         //tag
  ...
  for (i <- 1 until memWidth) {
    ...
    fregfile.io.write_ports(w_cnt).bits.data_securitytag := 0.U(2.W)     //default
  ...
  //val wdata_securitytag = UInt(2.W) 
  //wdata_securitytag := io.debug_wb_wdata      //tag
