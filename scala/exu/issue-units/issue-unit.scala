 ...
 *
 * @param issueWidth amount of things that can be issued  一次发射一条指令
 * @param numEntries size of issue queue
 * @param iqType type of issue queue
 */
case class IssueParams(
  dispatchWidth: Int = 1,               //一次派遣进rob一条指令
  issueWidth: Int = 1,                      //一次发射一条指令
  numEntries: Int = 8,                     //发射序列深度
  iqType: BigInt                                 //发射序列类型（有序/无序）
)
...
{
  // invalid  : slot holds no valid uop.     无效状态，即发射槽里没有有效的微指令
  // s_valid_1: slot holds a valid uop.    发射槽有一个有效的微指令
  // s_valid_2: slot holds a store-like uop that may be broken into two micro-ops.   发射槽里有类存储的微指令，需要分解为两个微操作
  ...
  (implicit p: Parameters) extends BoomBundle
{
  ...
  val dis_uops_securitytag         = Input(Vec(dispatchWidth,UInt(2.W)))             //派遣进入发射槽的指令和tag

  val iss_valids       = Output(Vec(issueWidth, Bool()))
  val iss_uops         = Output(Vec(issueWidth, new MicroOp()))

  val iss_uops_securitytag         = Output(Vec(issueWidth, UInt(2.W)))                 //发射出的指令和tag
  ...
  with IssueUnitConstants
{
  ...
  val dis_uops_securitytag = Array.fill(dispatchWidth) {Wire(UInt(2.W))}         //tag

  for (w <- 0 until dispatchWidth) {
    dis_uops(w) := io.dis_uops(w).bits

    dis_uops_securitytag(w) := io.dis_uops_securitytag(w)                                    //tag
  ...
  val slots = for (i <- 0 until numIssueSlots) yield { val slot = Module(new IssueSlot(numWakeupPorts)); slot }    //发射槽
