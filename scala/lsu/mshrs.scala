  ...
  with HasL1HellaCacheParameters
{
  ...
  // s_refill_req      : Make a request for a new cache line           对新cacheline重新发起请求
...
//Security_Tag
class BoomMSHRFile_Security(implicit edge: TLEdgeOut, p: Parameters) extends BoomModule()(p)
  with HasL1HellaCacheParameters
{
  val io = IO(new Bundle {
    val req  = Flipped(Vec(memWidth, Decoupled(new BoomDCacheReqInternal_Security))) // Req from s2 of DCache pipe
    val req_is_probe = Input(Vec(memWidth, Bool()))
    val req_trust_tag = Input(Vec(memWidth, UInt(2.W)))
    val req_s2_hit_insecurity = Input(Vec(memWidth, Bool()))

    val req_s2_security_check_result = Input(Vec(memWidth,Bool()))

    val req_s2_hit_coherence  = Input(Vec(memWidth, Bool()))
    val req_s2_tag_match_way_coherence = Input(Vec(memWidth, Bits(nWays.W)))
    val req_s2_p_f_val = Input(Vec(memWidth, Bool()))
    val resp = Decoupled(new BoomDCacheResp_Security)
    val secondary_miss = Output(Vec(memWidth, Bool()))
    val block_hit = Output(Vec(memWidth, Bool()))
    ...
    val dcache_security_check_exception = Output(Bool())

    val mem_acquire  = Decoupled(new TLBundleA(edge.bundle))
    val mem_grant    = Flipped(Decoupled(new TLBundleD(edge.bundle)))
    val mem_finish   = Decoupled(new TLBundleE(edge.bundle))

    val refill     = Decoupled(new L1DataWriteReq_Security)
    val meta_write = Decoupled(new L1MetaWriteReq_Extension)
    val meta_p_f_write = Decoupled(new L1MetaRegWriteReq(1))
    val meta_read  = Decoupled(new L1MetaReadReq)
    val meta_resp  = Input(Valid(new L1Metadata))
    val replay     = Decoupled(new BoomDCacheReqInternal_Security_Extension2)
    val prefetch   = Decoupled(new BoomDCacheReq_Security)
    val wb_req     = Decoupled(new WritebackReq(edge.bundle))
  ...
  val req     = io.req(req_idx)
  val req_is_probe = io.req_is_probe(0)
  val req_trust_tag = io.req_trust_tag(req_idx)
  val req_s2_hit_insecurity = io.req_s2_hit_insecurity(req_idx)

  val req_s2_security_check_result = io.req_s2_security_check_result(req_idx)

  val req_s2_hit_coherence = io.req_s2_hit_coherence(req_idx)
  val req_s2_tag_match_way_coherence = io.req_s2_tag_match_way_coherence(req_idx)
  val req_s2_p_f_val = io.req_s2_p_f_val(req_idx)
  ...
  val sdq_rdy      = !sdq_val.andR
  val sdq_enq      = req.fire && cacheable && isWrite(req.bits.uop.mem_cmd)
  val sdq          = Mem(cfg.nSDQ, UInt((coreDataBits+coreDataBits/16).W))

  when (sdq_enq) {
    sdq(sdq_alloc_id) := Cat(req.bits.security_tag, req.bits.data)
  }

  // --------------------
  // The LineBuffer Data
  // Holds refilling lines, prefetched lines
  val lb = Mem(nLBEntries * cacheDataBeats, UInt((encRowBits+encRowBits/16).W))
  val lb_read_arb  = Module(new Arbiter(new LineBufferReadReq, cfg.nMSHRs))
  val lb_write_arb = Module(new Arbiter(new LineBufferWriteReq_Security, cfg.nMSHRs))

  lb_read_arb.io.out.ready  := false.B
  lb_write_arb.io.out.ready := true.B

  val lb_read_data = WireInit(0.U(encRowBits.W))
  val lb_read_security_tag = WireInit(0.U((encRowBits/16).W))
  when (lb_write_arb.io.out.fire) {
    lb.write(lb_write_arb.io.out.bits.lb_addr, Cat(lb_write_arb.io.out.bits.security_tag, lb_write_arb.io.out.bits.data))
  } .otherwise {
    lb_read_arb.io.out.ready := true.B
    when (lb_read_arb.io.out.fire) {
      lb_read_data := (lb.read(lb_read_arb.io.out.bits.lb_addr))(encRowBits-1, 0)
      lb_read_security_tag := (lb.read(lb_read_arb.io.out.bits.lb_addr))(encRowBits+encRowBits/16-1, encRowBits)
    }
  }
  def widthMap[T <: Data](f: Int => T) = VecInit((0 until memWidth).map(f))
  ...
  val meta_write_arb = Module(new Arbiter(new L1MetaWriteReq_Extension , cfg.nMSHRs))
  val meta_p_f_write_arb = Module(new Arbiter(new L1MetaRegWriteReq(1) , cfg.nMSHRs))
  val meta_read_arb  = Module(new Arbiter(new L1MetaReadReq            , cfg.nMSHRs))
  val data_read_coherence_arb = Module(new Arbiter(new L1DataReadReq   , cfg.nMSHRs))
  val wb_req_arb     = Module(new Arbiter(new WritebackReq(edge.bundle), cfg.nMSHRs))
  val replay_arb     = Module(new Arbiter(new BoomDCacheReqInternal_Security_Extension2 , cfg.nMSHRs))
  val resp_arb       = Module(new Arbiter(new BoomDCacheResp_Security  , cfg.nMSHRs + nIOMSHRs))
  val refill_arb     = Module(new Arbiter(new L1DataWriteReq_Security  , cfg.nMSHRs))

  val commit_vals    = Wire(Vec(cfg.nMSHRs, Bool()))
  val commit_addrs   = Wire(Vec(cfg.nMSHRs, UInt(coreMaxAddrBits.W)))
  val commit_cohs    = Wire(Vec(cfg.nMSHRs, new ClientMetadata))
  val dcache_security_check_exception = Wire(Vec(cfg.nMSHRs, Bool()))
  ...
  val pri_val = req.valid && sdq_rdy && cacheable && !idx_match(req_idx)
  val mshrs = (0 until cfg.nMSHRs) map { i =>
    val mshr = Module(new BoomMSHR_Security)
    mshr.io.id := i.U(log2Ceil(cfg.nMSHRs).W)
    ...
    mshr.io.req_is_probe := req_is_probe
    mshr.io.req.sdq_id   := sdq_alloc_id
    mshr.io.req_trust_tag := req_trust_tag
    mshr.io.req_s2_hit_insecurity := req_s2_hit_insecurity

    mshr.io.req_s2_security_check_result := req_s2_security_check_result

    mshr.io.req_s2_hit_coherence := req_s2_hit_coherence
    mshr.io.req_s2_tag_match_way_coherence := req_s2_tag_match_way_coherence
    mshr.io.req_s2_p_f_val := req_s2_p_f_val
    ...
    meta_write_arb.io.in(i) <> mshr.io.meta_write
    meta_p_f_write_arb.io.in(i) <> mshr.io.meta_p_f_write
    meta_read_arb.io.in(i)  <> mshr.io.meta_read
    mshr.io.meta_resp       := io.meta_resp
    data_read_coherence_arb.io.in(i) <> mshr.io.data_read_coherence
    mshr.io.data_resp_coherence      := io.data_resp_coherence
    wb_req_arb.io.in(i)     <> mshr.io.wb_req
    replay_arb.io.in(i)     <> mshr.io.replay
    refill_arb.io.in(i)     <> mshr.io.refill
    ...
    lb_read_arb.io.in(i)       <> mshr.io.lb_read
    mshr.io.lb_resp            := lb_read_data
    mshr.io.lb_resp_security   := lb_read_security_tag
    lb_write_arb.io.in(i)      <> mshr.io.lb_write

    commit_vals(i)  := mshr.io.commit_val
    commit_addrs(i) := mshr.io.commit_addr
    commit_cohs(i)  := mshr.io.commit_coh
    dcache_security_check_exception(i) := mshr.io.dcache_security_check_exception
  ...
  io.meta_write <> meta_write_arb.io.out
  io.meta_p_f_write <> meta_p_f_write_arb.io.out
  io.meta_read  <> meta_read_arb.io.out
  io.data_read_coherence <> data_read_coherence_arb.io.out
  io.wb_req     <> wb_req_arb.io.out
  ...
  val mmios = (0 until nIOMSHRs) map { i =>
    val id = cfg.nMSHRs + 1 + i // +1 for wb unit
    val mshr = Module(new BoomIOMSHR_Security(id))
  ...
  val respq = Module(new BranchKillableQueue(new BoomDCacheResp_Security, 4, u => u.uses_ldq, flow = false))
  respq.io.brupdate := io.brupdate
  respq.io.flush    := io.exception || (dcache_security_check_exception.asUInt).orR
  respq.io.enq      <> resp_arb.io.out
  io.resp           <> respq.io.deq
  io.dcache_security_check_exception := (dcache_security_check_exception.asUInt).orR
  ...
  io.replay <> replay_arb.io.out
  io.replay.bits.data := (sdq(replay_arb.io.out.bits.sdq_id))(coreDataBits-1, 0)
  io.replay.bits.security_tag := (sdq(replay_arb.io.out.bits.sdq_id))(coreDataBits+coreDataBits/16-1, coreDataBits)
...
class BoomMSHR_Security(implicit edge: TLEdgeOut, p: Parameters) extends BoomModule()(p)
  with HasL1HellaCacheParameters
{
  val io = IO(new Bundle {
    val id = Input(UInt())
    ...
    val req          = Input(new BoomDCacheReqInternal_Security)
    val req_is_probe = Input(Bool())
    val req_trust_tag = Input(UInt(2.W))
    val req_s2_hit_insecurity = Input(Bool())

    val req_s2_security_check_result = Input(Bool())                                    

    val req_s2_hit_coherence = Input(Bool())
    val req_s2_tag_match_way_coherence = Input(Bits(nWays.W))
    val req_s2_p_f_val = Input(Bool())
    val data_read_coherence = Decoupled(new L1DataReadReq)
    val data_resp_coherence = Input(Valid(UInt((encRowBits+encRowBits/16).W)))  
    ...
    val lb_read       = Decoupled(new LineBufferReadReq)
    val lb_resp       = Input(UInt(encRowBits.W))
    val lb_resp_security = Input(UInt((encRowBits/16).W))
    val lb_write      = Decoupled(new LineBufferWriteReq_Security)
  ...
  val s_data_read_coherence :: s_data_resp_coherence1 :: s_data_resp_coherence2 :: s_lb_write :: Nil = Enum(4)
  val state_coherence = RegInit(s_data_read_coherence)

  val req     = Reg(new BoomDCacheReqInternal_Security)
  val req_idx = req.addr(untagBits-1, blockOffBits)
  val req_tag = req.addr >> untagBits
  val req_block_addr = (req.addr >> blockOffBits) << blockOffBits
  val req_needs_wb = RegInit(false.B)
  val req_s2_hit_insecurity = Reg(Bool())

  val req_s2_security_check_result = Reg(Bool())

  val req_s2_hit_coherence = Reg(Bool())
  val req_s2_tag_match_way_coherence = Reg(Bits(nWays.W))
  val req_s2_p_f_val = Reg(Bool())
  ...
  val sec_rdy = (!cmd_requires_second_acquire && !io.req_is_probe &&
                 !state.isOneOf(s_invalid, s_meta_write_req, s_mem_finish_1, s_mem_finish_2))// Always accept secondary misses
  val coherence_done = WireInit(false.B)
  val refill_done_reg = Reg(Bool())

  val dcache_security_check_exception = RegInit(false.B)
  io.dcache_security_check_exception := dcache_security_check_exception

  val rpq = Module(new BranchKillableQueue(new BoomDCacheReqInternal_Security_Extension1, cfg.nRPQ, u => u.uses_ldq, false))
  rpq.io.brupdate := io.brupdate
  rpq.io.flush  := io.exception || dcache_security_check_exception
  assert(!(state === s_invalid && !rpq.io.empty))
  ...
  rpq.io.enq.valid := ((io.req_pri_val && io.req_pri_rdy) || (io.req_sec_val && io.req_sec_rdy)) && !isPrefetch(io.req.uop.mem_cmd)
  rpq.io.enq.bits  <> io.req
  rpq.io.enq.bits.req_trust_tag := io.req_trust_tag
  rpq.io.deq.ready := false.B


  val grantack = Reg(Valid(new TLBundleE(edge.bundle)))
  val refill_ctr  = Reg(UInt(log2Ceil(cacheDataBeats).W))
  val coherence_ctr = Reg(UInt((log2Ceil(cacheDataBeats)+1).W))
  val commit_line = Reg(Bool())
  val grant_had_data = Reg(Bool())
  val finish_to_prefetch = Reg(Bool())
  ...
  io.meta_write.valid    := false.B
  io.meta_p_f_write.valid := false.B
  io.req_pri_rdy         := false.B
  io.req_sec_rdy         := sec_rdy && rpq.io.enq.ready
  io.mem_acquire.valid   := false.B
  io.refill.valid        := false.B
  ...
  io.lb_write.valid      := false.B
  io.lb_read.valid       := false.B
  io.data_read_coherence.valid := false.B
  ...
  def handle_pri_req(old_state: UInt): UInt = {      //触发条件是miss了
    val new_state = WireInit(old_state)
    grantack.valid := false.B
    refill_ctr := 0.U
    coherence_ctr := 0.U
    assert(rpq.io.enq.ready)
    req := io.req
    req_s2_hit_insecurity := io.req_s2_hit_insecurity

    req_s2_security_check_result := io.req_s2_security_check_result

    req_s2_hit_coherence := io.req_s2_hit_coherence
    req_s2_tag_match_way_coherence := io.req_s2_tag_match_way_coherence
    req_s2_p_f_val := io.req_s2_p_f_val
    val old_coh   = io.req.old_meta.coh                                               //要被替换的cacheline meta信息
    req_needs_wb := old_coh.onCacheControl(M_FLUSH)._1 // does the line we are evicting need to be written back  逐出重填line
    when (io.req.tag_match && !io.req_s2_p_f_val && io.req_s2_security_check_result) {                   //miss 且 tag_match，更新（逐出重填）完会变化，这里应该加入check_result通过了才允许进
      val (is_hit, _, coh_on_hit) = old_coh.onAccess(io.req.uop.mem_cmd)    //is_hit是有无权限访问，是当前最新的数据
      when (is_hit) { // set dirty bit                              //如果有权限访问（那应该直接就写了），所以这里只能是读
        assert(isWrite(io.req.uop.mem_cmd))        
        new_coh     := coh_on_hit                                  //dirty bit
        new_state   := s_drain_rpq                               //重新走流水线就行
      } .otherwise { // upgrade permissions           //如果不是最新的，则需要从别的cache/mem里拿，再上流水线
        new_coh     := old_coh
        new_state   := s_refill_req
      }
    } .otherwise { // refill and writeback if necessary
  ...
  when (state === s_invalid) {
    ...
    io.mem_acquire.bits  := edge.AcquireBlock(
      ...
      lgSize          = lgCacheBlockBytes.U,
      growPermissions = grow_param)._2
    when (io.mem_acquire.fire) {               //给其他cache通知一致性改变
      state := s_refill_resp
    }
    refill_done_reg := false.B
  } .elsewhen (state === s_refill_resp) {
    when (edge.hasData(io.mem_grant.bits) && !req_s2_hit_coherence) {
      io.mem_grant.ready      := io.lb_write.ready
      io.lb_write.valid       := io.mem_grant.valid
      io.lb_write.bits.id     := io.id
      io.lb_write.bits.offset := refill_address_inc >> rowOffBits
      io.lb_write.bits.data   := io.mem_grant.bits.data(127,0)
      io.lb_write.bits.security_tag := io.mem_grant.bits.data(135,128)
    } .otherwise {
      io.mem_grant.ready      := true.B
    }
    ...
    when (((refill_done || refill_done_reg) && !req_s2_hit_coherence) || ((refill_done || refill_done_reg) && req_s2_hit_coherence && coherence_done)) {
      state := Mux(!grant_had_data || (req_s2_p_f_val && req_s2_hit_insecurity), s_drain_rpq, s_drain_rpq_loads)
      assert(!(!grant_had_data && req_needs_wb))
      commit_line := false.B

    }
    assert(!(refill_done && refill_done_reg), "refill_done and refill_done_reg don't cooperate well with each other");
    when (refill_done) {
      grantack.valid := edge.isRequest(io.mem_grant.bits)
      grantack.bits := edge.GrantAck(io.mem_grant.bits)
      new_coh := coh_on_grant
      refill_done_reg := true.B
    }
  } .elsewhen (state === s_drain_rpq_loads) {
    val drain_load = (isRead(rpq.io.deq.bits.uop.mem_cmd) &&
                     !isWrite(rpq.io.deq.bits.uop.mem_cmd) &&
    ...
    val word_idx  = if (rowWords == 1) 0.U else rp_addr(log2Up(rowWords*coreDataBytes)-1, log2Up(wordBytes))
    val data      = io.lb_resp
    val security_tag = io.lb_resp_security
    val data_word = data >> Cat(word_idx, 0.U(log2Up(coreDataBits).W))
    val security_tag_word = security_tag >> Cat(word_idx, 0.U(log2Up(coreDataBits/16).W))
    val loadgen = new LoadGen(rpq.io.deq.bits.uop.mem_size, rpq.io.deq.bits.uop.mem_signed,
      Cat(req_tag, req_idx, rpq.io.deq.bits.addr(blockOffBits-1,0)),
      data_word, false.B, wordBytes)
    val loadgen_security = new LoadGen_Security(rpq.io.deq.bits.uop.mem_size,
      Cat(req_tag, req_idx, rpq.io.deq.bits.addr(blockOffBits-1,0)),
      security_tag_word, false.B, wordBytes)
    val checker = (0 until 2).map { b => new DCache_Security_Check(rpq.io.deq.bits.req_trust_tag, loadgen_security.data(2*b+1, 2*b))}
    val security_check_result = (0 until 2).map { b => Mux(req_s2_hit_insecurity && req_needs_wb, false.B, checker(b).check)}       //如果hit且需要写回，则说明在另一个阵列里且安全检查不通过？那么一定是exception
    dcache_security_check_exception := rpq.io.deq.valid && io.lb_read.fire && drain_load && !(security_check_result(0) && security_check_result(1))

    when(rpq.io.deq.valid && io.lb_read.fire && drain_load && !(security_check_result(0) && security_check_result(1))) {                  //打印异常信息
        when(!(security_check_result(0))) {
          printf("dcache_security_check_exception:\n")
          printf(p"paddr = ${rpq.io.deq.bits.addr}\n")
          //printf(p"maddr_tag = ${loadgen_security.data(1, 0)}\n")
        } 
        when(!(security_check_result(1))) {
          printf("dcache_security_check_exception\n")
          printf(p"paddr = ${rpq.io.deq.bits.addr}\n")
          //printf(p"maddr_tag = ${loadgen_security.data(3, 2)}\n")             //这里只是mem的数据tag，但并不一定是想要的那个数据的tag（可能在另一个阵列里）
        }
        }
    ...
    io.resp.bits.uop  := rpq.io.deq.bits.uop
    io.resp.bits.data := loadgen.data
    io.resp.bits.security_tag := loadgen_security.data
    io.resp.bits.is_hella := rpq.io.deq.bits.is_hella
    when (rpq.io.deq.fire) {
      commit_line   := true.B
    }
    ...
    io.meta_write.bits.data.tag := req_tag
    io.meta_write.bits.way_en   := req.way_en
    io.meta_p_f_write.valid       := true.B
    io.meta_p_f_write.bits.idx    := req_idx
    io.meta_p_f_write.bits.way_en := req.way_en
    io.meta_p_f_write.bits.data   := false.B

    when (io.meta_write.fire && io.meta_p_f_write.fire) {
      state      := s_wb_req
    }
  } .elsewhen (state === s_wb_req) {
    io.wb_req.valid          := true.B
    ...
    val security_check_result = Mux(req_s2_hit_insecurity && req_needs_wb, false.B, true.B)
    dcache_security_check_exception := io.lb_read.fire && !security_check_result

    io.refill.valid       := io.lb_read.fire && !dcache_security_check_exception
    io.refill.bits.addr   := req_block_addr | (refill_ctr << rowOffBits)
    io.refill.bits.way_en := req.way_en
    io.refill.bits.wmask  := ~(0.U(rowWords.W))
    io.refill.bits.data   := io.lb_resp
    io.refill.bits.security_tag := io.lb_resp_security
    when (io.refill.fire) {
      refill_ctr := refill_ctr + 1.U
      when (refill_ctr === (cacheDataBeats - 1).U) {
        state := s_drain_rpq
    ...
    io.replay.ready <> rpq.io.deq.ready
    io.replay.valid <> rpq.io.deq.valid
    io.replay.bits <> rpq.io.deq.bits
    io.replay.bits.way_en    := req.way_en
    io.replay.bits.addr := Cat(req_tag, req_idx, rpq.io.deq.bits.addr(blockOffBits-1,0))
    io.replay.bits.way_en_coherence := req_s2_tag_match_way_coherence
    when (io.replay.fire && isWrite(rpq.io.deq.bits.uop.mem_cmd)) {
      // Set dirty bit
      val (is_hit, _, coh_on_hit) = new_coh.onAccess(rpq.io.deq.bits.uop.mem_cmd)
      assert(is_hit, "We still don't have permissions for this store")
    ...
    io.meta_write.valid         := Mux(req_s2_p_f_val && req_s2_hit_insecurity && req_needs_wb, false.B, true.B)
    io.meta_write.bits.idx      := req_idx
    io.meta_write.bits.data.coh := new_coh
    io.meta_write.bits.data.tag := req_tag
    io.meta_write.bits.way_en   := req.way_en
    io.meta_write.bits.way_en_coherence := req_s2_tag_match_way_coherence
    io.meta_write.bits.hit_coherence := req_s2_hit_coherence
    io.meta_p_f_write.valid       := true.B
    io.meta_p_f_write.bits.idx    := req_idx
    io.meta_p_f_write.bits.way_en := req.way_en
    io.meta_p_f_write.bits.data   := false.B
    when (Mux(req_s2_p_f_val && req_s2_hit_insecurity && req_needs_wb, io.meta_p_f_write.fire, io.meta_write.fire && io.meta_p_f_write.fire)) {
      state := s_mem_finish_1
      finish_to_prefetch := false.B
    }
  } .elsewhen (state === s_mem_finish_1) {
  ...
  val data_resp_coherence_temp = Reg(UInt((encRowBits+encRowBits/16).W))
  when (((state === s_refill_req) || (state === s_refill_resp)) && req_s2_hit_coherence) {
    when (state_coherence === s_data_read_coherence){
      io.data_read_coherence.bits.way_en := req_s2_tag_match_way_coherence
      io.data_read_coherence.bits.addr := req_block_addr | (coherence_ctr << rowOffBits)
      when (coherence_ctr <= (cacheDataBeats - 1).U) {
        io.data_read_coherence.valid := true.B
        when (io.data_read_coherence.fire) {
          state_coherence := s_data_resp_coherence1;
        }
      }.otherwise {
        coherence_done := true.B
      }
      ...
      data_resp_coherence_temp := Mux(io.data_resp_coherence.valid, io.data_resp_coherence.bits, data_resp_coherence_temp)
      state_coherence := Mux(io.data_resp_coherence.valid, s_lb_write, s_data_resp_coherence2)
    ...
    } .elsewhen(state_coherence === s_lb_write)  {
      io.lb_write.valid       := true.B
      io.lb_write.bits.id     := io.id
      io.lb_write.bits.offset := coherence_ctr
      io.lb_write.bits.data   := data_resp_coherence_temp(encRowBits-1, 0)
      io.lb_write.bits.security_tag := data_resp_coherence_temp(encRowBits+encRowBits/16-1, encRowBits)
      when (io.lb_write.fire) {
        coherence_ctr := coherence_ctr + 1.U
...
class LineBufferWriteReq_Security(implicit p: Parameters) extends LineBufferReadReq()(p)
{
  val data   = UInt(encRowBits.W)
  val security_tag   = UInt((encRowBits/16).W)
}

class BoomDCacheReqInternal_Security(implicit p: Parameters) extends BoomDCacheReq_Security()(p)
  with HasL1HellaCacheParameters
{
  // miss info
  val tag_match = Bool()
  ...
  val req_is_store_security_tag = Bool()
}

class BoomIOMSHR_Security(id: Int)(implicit edge: TLEdgeOut, p: Parameters) extends BoomModule()(p)
  with HasL1HellaCacheParameters
{
  val io = IO(new Bundle {
    val req  = Flipped(Decoupled(new BoomDCacheReq_Security))
    val resp = Decoupled(new BoomDCacheResp_Security)
    val mem_access = Decoupled(new TLBundleA(edge.bundle))
    val mem_ack    = Flipped(Valid(new TLBundleD(edge.bundle)))
  ...
  def beatOffset(addr: UInt) = addr.extract(beatOffBits-1, wordOffBits)                             //wordofbits = 64

  def wordFromBeat(addr: UInt, dat: UInt) = {
    val shift = Cat(beatOffset(addr), 0.U((wordOffBits+log2Ceil(wordBytes)).W))             //wordBytes = 8 
    (dat >> shift)(wordBits-1, 0)
  }

  def word_securityFromBeat(addr:UInt, dat:UInt) = {
    val shift = Cat(beatOffset(addr), 0.U((wordOffBits+log2Ceil(wordBytes)).W))              //shift = 0/64
    val tag = Mux (shift === 0.U, dat(131,128),  dat(135,132))                                                 
    tag
  }

  val req = Reg(new BoomDCacheReq_Security)
  val grant_word = Reg(UInt(wordBits.W))
  val grant_word_security = Reg(UInt((wordBits/16).W))
  ...
  val loadgen = new LoadGen(req.uop.mem_size, req.uop.mem_signed, req.addr, grant_word, false.B, wordBytes)
  val loadgen_security = new LoadGen_Security(req.uop.mem_size, req.addr, grant_word_security, false.B, wordBytes)
  ...
  when (state === s_mem_ack && io.mem_ack.valid) {
    state := s_resp
    when (isRead(req.uop.mem_cmd)) {
      grant_word := wordFromBeat(req.addr, io.mem_ack.bits.data)                                             //返回对应的64 bit data
      grant_word_security := word_securityFromBeat(req.addr, io.mem_ack.bits.data)       //返回对应的 4 bit  tag
    }
  }
  when (state === s_resp) {
    when (!send_resp || io.resp.fire) {
...
class BoomDCacheReqInternal_Security_Extension1(implicit p: Parameters) extends BoomDCacheReqInternal_Security()(p)
{
  val req_trust_tag = UInt(2.W)
}

class BoomDCacheReqInternal_Security_Extension2(implicit p: Parameters) extends BoomDCacheReqInternal_Security()(p)
{
  val way_en_coherence = Bits(nWays.W)
}

class L1MetaWriteReq_Extension(implicit p: Parameters) extends L1MetaWriteReq()(p){
  val way_en_coherence = Bits(nWays.W)
  val hit_coherence = Bool()
}
