...
class LSUExeIO(implicit p: Parameters) extends BoomBundle()(p)               //lsu和exe的交互
{
  // The "resp" of the maddrcalc is really a "req" to the LSU     //存储地址计算单元的resp就是lsu的req（针对exe）
  val req       = Flipped(new ValidIO(new FuncUnitResp(xLen)))           //包含securitytag 
  // Send load data to regfiles
  val iresp    = new DecoupledIO(new boom.exu.ExeUnitResp(xLen))      //浮点/整型 输出
  val fresp    = new DecoupledIO(new boom.exu.ExeUnitResp(xLen+1)) // TODO: Should this be fLen? 
}

class BoomDCacheReq(implicit p: Parameters) extends BoomBundle()(p)    //boom dcache的输入
  with HasBoomUOP
{
  val addr  = UInt(coreMaxAddrBits.W)                 //地址

 // val addr_securitytag = UInt(2.W)   //tag，应该是不需要给dcache传入tag

  val data  = Bits(coreDataBits.W)                           //存的数据

  val data_securitytag = UInt(2.W)   //tag

  val is_hella = Bool() // Is this the hellacache req? If so this is not tracked in LDQ or STQ，hella好像是一种io接口名称
}

class BoomDCacheResp(implicit p: Parameters) extends BoomBundle()(p)     //dcache的输出
  with HasBoomUOP
{
...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)      //lsu与dcache
{
  // In LSU's dmem stage, send the request
  val req         = new DecoupledIO(Vec(memWidth, Valid(new BoomDCacheReq)))         //lsu到dcache
  ...
  val resp        = Flipped(Vec(memWidth, new ValidIO(new BoomDCacheResp)))             //dcache到lsu
...
class LSUCoreIO(implicit p: Parameters) extends BoomBundle()(p)          //lsu与core
{
  val exe = Vec(memWidth, new LSUExeIO)                                 //为功能单元与lsu的交互，addr及addr_securitytag都已加入              

  val dis_uops    = Flipped(Vec(coreWidth, Valid(new MicroOp)))

  val dis_uops_securitytag = Input(UInt(2.W))                                  //tag
...
class LSUIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)    //直接去看最下边带securitytag的
{
...
class LDQEntry(implicit p: Parameters) extends BoomBundle()(p)               //load queue entry
    with HasBoomUOP
{
...
class STQEntry(implicit p: Parameters) extends BoomBundle()(p)        //store
   with HasBoomUOP
{
  val addr                = Valid(UInt(coreMaxAddrBits.W))
  val addr_is_virtual     = Bool() // Virtual address, we got a TLB miss
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  val io = IO(new LSUIO)
  */
  val io = IO(new LSUIO_Security)                                      //加入tag
  for (w <- 0 until memWidth) {
    val temp = io.dmem.resp(w).bits.security_tag
    dontTouch(temp)
  }
  val exception_temp = io.dmem.dcache_security_check_exception                  //data_tag_check
  dontTouch(exception_temp)
 
  val ldq = Reg(Vec(numLdqEntries, Valid(new LDQEntry_Security)))
  val stq = Reg(Vec(numStqEntries, Valid(new STQEntry_Security)))
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    when (dis_ld_val)
    {
      ...
      ldq(ld_enq_idx).bits.req_trust_tag   := io.core.dis_uops_security(w)                                    //load指令的tag
      ...
      .elsewhen (dis_st_val)
    {
      ...
      stq(st_enq_idx).bits.req_trust_tag := io.core.dis_uops_security(w)                                    //store指令的tag
      stq(st_enq_idx).bits.addr.valid := false.B
      // stq(st_enq_idx).bits.data.valid := false.B
      stq(st_enq_idx).bits.data.valid := Mux(io.core.dis_uops(w).bits.lrs2_rtype === RT_X, true.B, false.B)
      when (io.core.dis_uops(w).bits.lrs2_rtype === RT_X) {
        stq(st_enq_idx).bits.data.bits := io.core.dis_uops(w).bits.prs2                                           //需要存入的数据所在的寄存器

        stq(st_enq_idx).bits.data_securitytag := io.core.dis_uops_security(w)                          //这里后续会被改成真正的数据tag

      }
      stq(st_enq_idx).bits.committed  := false.B
      stq(st_enq_idx).bits.succeeded  := false.B
      stq(st_enq_idx).bits.store_security_tag  := (io.core.dis_uops(w).bits.lrs2_rtype === RT_X)                //swt指令标志
  ...
  val exe_req = WireInit(VecInit(io.core.exe.map(_.req)))                                    //addr及addr_tag都在这里
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  val stq_commit_e  = stq(stq_execute_head)               //提交的stq
  ...
  val exe_tlb_vaddr = widthMap(w =>                                                                                                //地址转换
                    Mux(will_fire_load_incoming (w) ||
                        will_fire_stad_incoming (w) ||
  ...
  //tlb用于把虚拟地址转换为物理地址，这里addr_tag应该不需要处理，转换完的addr和addr_tag一起存入ldq/stq
  ...
  val exe_is_store_security_tag = widthMap(w =>                                //是否需要存tag
                   Mux(will_fire_stad_incoming (w) ||
                       will_fire_sta_incoming  (w) ||
                       will_fire_sta_retry     (w)  , stq(exe_tlb_uop(w).stq_idx).bits.store_security_tag,
                                                      false.B))   

  /*val exe_is_store_security_tag = widthMap(w =>
                   Mux(will_fire_stad_incoming (w) ||
                       will_fire_sta_incoming  (w) ||
                       will_fire_sta_retry     (w)  ,exe_req(w).bits.addr_securitytag,                  //addr_securitytag
                                                      false.B))           */                                           
  ...
  for (w <- 0 until memWidth) {
    ...
    dtlb.io.req(w).bits.is_store_security_tag := exe_is_store_security_tag(w)
  }
  ...
  val exe_tlb_paddr = widthMap(w => Cat(dtlb.io.resp(w).paddr(paddrBits-1,corePgIdxBits),                                                 //物理地址
                                        exe_tlb_vaddr(w)(corePgIdxBits-1,0)))
  val exe_tlb_uncacheable = widthMap(w => !(dtlb.io.resp(w).cacheable))
  val exe_tlb_req_trsut_tag = widthMap(w => ldq(exe_tlb_uop(w).ldq_idx).bits.req_trust_tag)                                              //securitytag传递
  ...
  val dmem_req_trust_tag = Wire(Vec(memWidth, UInt(2.W)))
  val dmem_req_is_store_security_tag = Wire(Bool())
  val dmem_domain_trust_tag = Mux(dmem_req(0).valid, dmem_req_trust_tag(0) =/= 0.U(2.W), dmem_req_trust_tag(1) =/= 0.U(2.W))
  val req_trust_tag = RegInit(0.U(2.W))
  val domain_trust_tag = RegInit(false.B)
  val domain_switch = RegInit(false.B)
  val domain_switch_ready = !(domain_switch && (io.dmem.dcache_s1_valid.asUInt.orR || io.dmem.dcache_s2_valid.asUInt.orR))
  val last_domain_trust_tag = RegNext(io.dmem.domain_trust_tag)
  io.dmem.req.valid := dmem_req.map(_.valid).reduce(_||_) && (dmem_domain_trust_tag === domain_trust_tag) && domain_switch_ready
  /*
  io.dmem.req.valid := dmem_req.map(_.valid).reduce(_||_)
  io.dmem.req.bits  := dmem_req
  */
  for (w <- 0 until memWidth) {
    io.dmem.req.bits(w).valid  := dmem_req(w).valid
    io.dmem.req.bits(w).bits   <> dmem_req(w).bits
    //io.dmem.req.bits(w).bits.security_tag := dmem_req(w).bits.data_securitytag                                      //tag = data_tag

   io.dmem.req.bits(w).bits.security_tag := dmem_req_trust_tag(w) ## dmem_req_trust_tag(w)        //tag = uop_tag

    io.dmem.req_trust_tag(w)   := dmem_req_trust_tag(w)
  }
  io.dmem.req_is_store_security_tag := dmem_req_is_store_security_tag
  ...
  assert(!(dmem_req(0).valid && dmem_req(1).valid && (dmem_req_trust_tag(0) =/= dmem_req_trust_tag(1))), "dmem_req_trust_tag(0) does not match with dmem_req_trust_tag(1)")
  when (io.dmem.req.ready && dmem_req.map(_.valid).reduce(_||_)) {
    req_trust_tag := Mux(dmem_req(0).valid, dmem_req_trust_tag(0), dmem_req_trust_tag(1))
    domain_trust_tag := dmem_domain_trust_tag
    when (dmem_domain_trust_tag =/= domain_trust_tag) {
      domain_switch := true.B
    }
  }
  assert((domain_trust_tag === false.B && req_trust_tag === 0.U(2.W)) || (domain_trust_tag === true.B && req_trust_tag =/= 0.U(2.W)), "req_trust_tag does not match with domain_trust_tag at register")
  io.dmem.domain_trust_tag := Mux(domain_switch_ready, domain_trust_tag, last_domain_trust_tag)
  for (w <- 0 until memWidth) {
    assert(!(io.dmem.req.fire && io.dmem.req.bits(w).valid && (((io.dmem.domain_trust_tag === false.B) && (io.dmem.req_trust_tag(w) =/= 0.U(2.W))) || ((io.dmem.domain_trust_tag === true.B) && (io.dmem.req_trust_tag(w) === 0.U(2.W))))), "req_trust_tag does not match with domain_trust_tag at ports")
  }
  when (domain_switch && domain_switch_ready) {
    domain_switch := false.B
  }
  
  when (!will_fire_load_incoming(0) && !will_fire_load_retry(0) && will_fire_store_commit(0)) {
    dmem_req_is_store_security_tag := stq_commit_e.bits.store_security_tag
  } .otherwise {
    dmem_req_is_store_security_tag := false.B
  }
  ...
  for (w <- 0 until memWidth) {
    ...
    dmem_req(w).bits.data_securitytag  := 0.U(2.W)     //default
 
    dmem_req(w).bits.is_hella := false.B
    dmem_req_trust_tag(w)  := 0.U

    io.dmem.s1_kill(w) := false.B

    when (will_fire_load_incoming(w)) {
      dmem_req(w).valid      := !exe_tlb_miss(w) && !exe_tlb_uncacheable(w) && (!exe_tlb_uop(w).br_mask.orR || (exe_tlb_req_trsut_tag(w) === req_trust_tag))
      dmem_req(w).bits.addr  := exe_tlb_paddr(w)
      dmem_req(w).bits.uop   := exe_tlb_uop(w)
      dmem_req_trust_tag(w)  := exe_tlb_req_trsut_tag(w)
      ...
      //                               coreDataBytes)).data
      dmem_req(w).bits.data     := Mux(stq_commit_e.bits.store_security_tag,                 
                                    stq_commit_e.bits.data.bits,
                                    (new freechips.rocketchip.rocket.StoreGen(                                                        //这里storegen是生成存入的data的，应该搞一个对应的store
                                    stq_commit_e.bits.uop.mem_size, 0.U,
                                    stq_commit_e.bits.data.bits,
                                    coreDataBytes)).data)

      dmem_req(w).bits.data_securitytag    := stq_commit_e.bits.data_securitytag              //tag对应

      dmem_req(w).bits.uop      := stq_commit_e.bits.uop
      dmem_req_trust_tag(w)     := stq_commit_e.bits.req_trust_tag
      ...
      dmem_req(w).valid      := true.B && (!ldq_wakeup_e.bits.uop.br_mask.orR || (ldq_wakeup_e.bits.req_trust_tag === req_trust_tag))
      dmem_req(w).bits.addr  := ldq_wakeup_e.bits.addr.bits
      dmem_req(w).bits.uop   := ldq_wakeup_e.bits.uop
      dmem_req_trust_tag(w)  := ldq_wakeup_e.bits.req_trust_tag
      ...
      dmem_req_trust_tag(w)           := req_trust_tag
    ...
    when (will_fire_load_incoming(w) || will_fire_load_retry(w))
    {
      ...
      ldq(ldq_idx).bits.addr.bits           := Mux(exe_tlb_miss(w), exe_tlb_vaddr(w), exe_tlb_paddr(w))                     //load地址

      //ldq(ldq_idx).bits.req_trust_tag   := exe_req(w).bits.addr_securitytag                                                                          //load的securitytag
    ...
    when (will_fire_sta_incoming(w) || will_fire_stad_incoming(w) || will_fire_sta_retry(w))
    {
      ...
      stq(stq_idx).bits.addr.bits  := Mux(exe_tlb_miss(w), exe_tlb_vaddr(w), exe_tlb_paddr(w))                               //store地址

      //stq(stq_idx).bits.req_trust_tag   := exe_req(w).bits.addr_securitytag                                                                          //store的securitytag
    ...
    when (will_fire_std_incoming(w) || will_fire_stad_incoming(w) || fp_stdata_fire)
    {
      ...
      when (!(stq(sidx).bits.data.valid)) {
        stq(sidx).bits.data.valid := true.B
        stq(sidx).bits.data.bits  := Mux(will_fire_std_incoming(w) || will_fire_stad_incoming(w),                       //真正存入的data
          exe_req(w).bits.data,
          io.core.fp_stdata.bits.data)

        stq(sidx).bits.data_securitytag  := Mux(will_fire_std_incoming(w) || will_fire_stad_incoming(w),                       //真正存入的data_tag
          exe_req(w).bits.data_securitytag,
          io.core.fp_stdata.bits.data_securitytag)                //不支持，只是做了个接口
  ...
  val mem_ldq_e            = widthMap(w =>
                             ...
                             Mux(fired_load_wakeup  (w), mem_ldq_wakeup_e, (0.U).asTypeOf(Valid(new LDQEntry_Security))))))
  val mem_stq_e            = widthMap(w =>
                             Mux(fired_stad_incoming(w) ||
                                 fired_sta_incoming (w), mem_stq_incoming_e(w),
                             Mux(fired_sta_retry    (w), mem_stq_retry_e, (0.U).asTypeOf(Valid(new STQEntry_Security)))))
 ...
 //store的addr需要从tlb响应获取
  ...
  for (i <- 0 until numLdqEntries) {
    ...
    val l_addr  = ldq(i).bits.addr.bits                     //load的addr
  ...
  for (i <- 0 until numStqEntries) {
    val s_addr = stq(i).bits.addr.bits                  //store的addr
    ...
    for (w <- 0 until memWidth) {
      when (do_ld_search(w) && stq(i).valid && lcam_st_dep_mask(w)(i) && !stq(i).bits.store_security_tag) {
  ...
  //val r_xcpt_data_check_unpass  = RegInit(false.B)
 ...
 // io.core.lxcpt_data_check_unpass := r_xcpt_data_check_unpass
  ...
  for (w <- 0 until memWidth) {
    ...
    // Handle the response                                     //dcache的响应
    when (io.dmem.resp(w).valid)
    {
      when (io.dmem.resp(w).bits.uop.uses_ldq)                  //load
      {
        ...
        io.core.exe(w).iresp.bits.data_securitytag := io.dmem.resp(w).bits.security_tag         //tag
        ...
        ldq(ldq_idx).bits.debug_wb_data_securitytag  := io.dmem.resp(w).bits.security_tag   //tag

      }
        .elsewhen (io.dmem.resp(w).bits.uop.uses_stq)                 //store
      {
        assert(!io.dmem.resp(w).bits.is_hella)
        ...
        when (io.dmem.resp(w).bits.uop.is_amo) {
          ...
          stq(io.dmem.resp(w).bits.uop.stq_idx).bits.debug_wb_data_securitytag := io.dmem.resp(w).bits.security_tag      //tag
      ...
      .elsewhen (!dmem_resp_fired(w) && wb_forward_valid(w))
    {
      ...
      val loadgen  = new freechips.rocketchip.rocket.LoadGen(                                       //loadgen
                                forward_uop.mem_size, forward_uop.mem_signed,
                                wb_forward_ld_addr(w),
                                storegen.data, false.B, coreDataBytes)


      val loadgen_securitytag  =    stq_e.bits.data_securitytag                                     //tag(不确定对不对)
      ...
      io.core.exe(w).iresp.bits.data_securitytag := loadgen_securitytag                //tag
      ...
      when (data_ready && live) {
        ...
        ldq(f_idx).bits.debug_wb_data_securitytag   := loadgen_securitytag          //tag
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    if (MEMTRACE_PRINTF) {                                                    //好像只有打开这个功能才起作用，应该是个debug的功能
      when (commit_store || commit_load) {
        ...
        //val stdata_securitytag = Mux(commit_store, stq(idx).bits.data_securitytag, 0.U)                                       //tag
...
//Security_Tag
class LSUIO_Security(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)          //添加了securitytag的lsu
{
  val ptw   = new rocket.TLBPTWIO               //page table walk，即虚拟地址转化为物理地址
  val core  = new LSUCoreIO_Security         //lsu与core的交互，加入tag
  val dmem  = new LSUDMemIO_Security //lsu与dcache的交互，加入tag

  val hellacache = Flipped(new freechips.rocketchip.rocket.HellaCacheIO)
}

class LSUCoreIO_Security(implicit p: Parameters) extends BoomBundle()(p)
{
  val exe = Vec(memWidth, new LSUExeIO)                                                //和core交互，包括addr及addr_securitytag

  val dis_uops    = Flipped(Vec(coreWidth, Valid(new MicroOp)))     //输入指令的微码，corewidth为core一次处理指令的量（超标量宽度）
  val dis_uops_security = Input(Vec(coreWidth, UInt(2.W)))              //访存指令的securitytag
  val dis_ldq_idx = Output(Vec(coreWidth, UInt(ldqAddrSz.W)))
  val dis_stq_idx = Output(Vec(coreWidth, UInt(stqAddrSz.W)))
  ...
  val lsu_data_check_unpass = Output(Vec(memWidth, Bool()))          //lsu所完成的那部分data_check
  ...
  //val lxcpt_data_check_unpass = Output(Bool())                               //lsu_data_check_unpass
...
class LDQEntry_Security(implicit p: Parameters) extends BoomBundle()(p)
    with HasBoomUOP
{
  val addr                = Valid(UInt(coreMaxAddrBits.W))
  val addr_is_virtual     = Bool() // Virtual address, we got a TLB miss
  ...
  val debug_wb_data_securitytag = UInt(2.W)   //tag

  val req_trust_tag       = UInt(2.W)                       //load请求的tag
}

class STQEntry_Security(implicit p: Parameters) extends BoomBundle()(p)
   with HasBoomUOP
{
  val addr                = Valid(UInt(coreMaxAddrBits.W))
  val addr_is_virtual     = Bool() // Virtual address, we got a TLB miss 
  ...
  val data_securitytag = UInt(2.W)                //存入的tag
  ...
  val req_trust_tag       = UInt(2.W)               //securitytag
  val store_security_tag  = Bool()                //？
}
