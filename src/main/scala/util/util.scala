...
object ImmGen                                    //立即数生成
{
  ...
  def apply(ip: UInt, isel: UInt): SInt = {                                                                  //ip为输入的packed imme，isel为输入的imm sel信号
    val sign = ip(LONGEST_IMM_SZ-1).asSInt                                                       //符号位
    val i30_20 = Mux(isel === IS_U, ip(18,8).asSInt, sign)                                 //如果是u-type（无符号）指令，则30_20位由ip的18_8位组成（左移12位），否则即有符号数，进行符号位扩展
    val i19_12 = Mux(isel === IS_U || isel === IS_J, ip(7,0).asSInt, sign)     //同上
    ...
    return Cat(sign, i30_20, i19_12, i11, i10_5, i4_1, i0).asSInt                   //拼接为生成的imme
  }
}
