...
//------------------------------------------------------------------------------
// Rename Map Table
//重命名映射表，用于记录lreg和preg的映射关系
//重命名映射表里包括了一个map_table（即从逻辑寄存器到物理寄存器的映射，实现方式为一个寄存器向量）、
//一个br_snapshots用于记录每个推测分支的map_table快照（每个分支有一个完整的map_table）、
//一个remap_table用于记录修改map中间值（下一个时钟周期将会更新至map_table。
...
class MapReq(val lregSz: Int) extends Bundle         //map的输入（逻辑源操作数寄存器123，逻辑目的操作数寄存器）
{
...
class MapResp(val pregSz: Int) extends Bundle     //map的输出（物理源操作数寄存器123，旧的物理目的操作数寄存器）
{
...
{
  val pregSz = log2Ceil(numPregs)        //物理寄存器数量

  val io = IO(new BoomBundle()(p) {
    // Logical sources -> physical sources.          逻辑到物理的对应（注意只有源寄存器）
    //map_table的输入，将逻辑源寄存器发送给重命名映射表准备映射
    val map_reqs    = Input(Vec(plWidth, new MapReq(lregSz)))
    val map_resps   = Output(Vec(plWidth, new MapResp(pregSz)))

    // Remapping an ldst to a newly allocated pdst?          物理目的寄存器的修改
    //remap_table 的输入端口，用于记录新的目的寄存器，并在下个周期更新map_table
    val remap_reqs  = Input(Vec(plWidth, new RemapReq(lregSz, pregSz)))

    // Dispatching branches: need to take snapshots of table state.       分支指令需要快照
    val ren_br_tags = Input(Vec(plWidth, Valid(UInt(brTagSz.W))))

    // Signals for restoring state following misspeculation.     错误预测时候回滚
  ...
  //分支快照是指保存执行分支指令之前各个寄存器的值，在分支预测失误时用于恢复逻辑。

  // The intermediate states of the map table following modification by each pipeline slot.         
  val remap_table = Wire(Vec(plWidth+1, Vec(numLregs, UInt(pregSz.W))))                 //记录新的目的寄存器，并在下个周期更新map_table
  ...
  //最新的目的寄存器映射关系通过 remap_table 传入 map_table
  ...
  //scanLeft函数从左向右操作，操作由数据选择器 Mux(ldst, new_pdst, pdst) 实现,其中：
  //ldst 来自 remap_ldst_oh，标志是否需要重映射；
  //new_pdst 是第 i 个逻辑寄存器在 io.remap_reqs 中对应的的物理寄存器编码；
  //pdst 是上一次 scanLeft 操作的结果，初始值为 map_table(i)，即第 i 个逻辑目的寄存器在map_table 中对应的物理寄存器编码。
  ...
  //分支标签用于记录该指令与分支的依赖关系，当分支预测失误时，依赖于该分支的所有指令(通过分支标签检索) 将被清除，
  //其对寄存器的修改也将被恢复到执行分支指令前的状态。 
  ...
  // Read out mappings.
  //遍历所有指令，对第 i 条指令，对(0 until i)调用 foldLeft 方法，操作为一个选择器，其中：
  ...
  for (i <- 0 until plWidth) {
    ...
    if (!float) io.map_resps(i).prs3 := DontCare                   //对于非浮点操作，不需要使用 prs3。
  }
