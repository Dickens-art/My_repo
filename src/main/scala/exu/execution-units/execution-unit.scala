...
import freechips.rocketchip.rocket.{BP}             //freechips.rocketchip.rocket is in generator/rocket-chip/src/main/scala/rocket
 ...
 */
class ExeUnitResp(val dataWidth: Int)(implicit p: Parameters) extends BoomBundle          //执行单元的响应  --输出信号
  with HasBoomUOP
{
  ...
  val data_securitytag = Bits(2.W)                                                                //一个数据搭配一个tag（注意这里是64：2，因为这64 bits 是一个整体）

  //val data_check_unpass = Bits(1.W)                                                          //数据安全监测结果指示位

  val predicated = Bool() // Was this predicated off?
  val fflags = new ValidIO(new FFlagsResp) // write fflags to ROB // TODO: Do this better
}

class ExeUnitResp_securitytag(val dataWidth: Int)(implicit p: Parameters) extends BoomBundle          //执行单元的响应
  with HasBoomUOP
{
  val data = Bits(dataWidth.W)
...
class ExeUnitResp_securitytag_datacheck(val dataWidth: Int)(implicit p: Parameters) extends BoomBundle          //执行单元的响应
  with HasBoomUOP
{
  val data = Bits(dataWidth.W)
...
/**
 * Floating Point flag response                                                                       //浮点标志
 */
...
/**
 * Abstract Top level Execution Unit that wraps lower level functional units to make a    //抽象一个顶层的执行单元，其包含多个功能单元
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val io = IO(new Bundle {
    ...
    val req      = Flipped(new DecoupledIO(new FuncUnitReq(dataWidth)))         //单元的输入信号1
    ...
    val data_check_unpass    = if (writesIrf)  Output(Bool())  else null           //data_check_unpass--单元的输出信号1

    val bypass   = Output(Vec(numBypassStages, Valid(new ExeUnitResp(dataWidth))))          //每一级bypass的输出信号2
  ...
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  ...
  val out_str =
    BoomCoreStringPrefix("==ExeUnit==") +                                //输出aluexe单元的配置
  ...
  io.fu_types := Mux(hasAlu.B, FU_ALU, 0.U) |
                 ...
                 Mux(hasMem.B, FU_MEM, 0.U)                                                                // 赋值为1.U
  ...
  if (hasAlu) {
    ...
    alu.io.req.bits.uop_securitytag  := io.req.bits.uop_securitytag
    ...
    alu.io.req.bits.rs1_data_securitytag := io.req.bits.rs1_data_securitytag                            //tag
    alu.io.req.bits.rs2_data_securitytag := io.req.bits.rs2_data_securitytag
    alu.io.req.bits.rs3_data_securitytag := DontCare
  ...
  var rocc: RoCCShim = null                //rocc，tag不支持
  if (hasRocc) {
  ...
  // Pipelined, IMul Unit ------------------         流水线乘法单元，tag不支持
  ...
  var ifpu: IntToFPUnit = null                    //整型与浮点的转化。tag不支持
  if (hasIfpu) {
    ...
    queue.io.enq.bits.data_securitytag   := ifpu.io.resp.bits.data_securitytag
  ...
  if (hasMem) {
    ...
    io.lsu_io.req := maddrcalc.io.resp            //tag[p_addr]在这里，已经加入了addr_securitytag

    io.ll_iresp <> io.lsu_io.iresp                        //包含tag
    if (usingFPU) {
  ...
  //PriorityMux输出与最低索引断言选择信号相关的值

  if (writesIrf) {                                //写回寄存器的data和tag
    ...
    io.iresp.bits.data_securitytag := PriorityMux(iresp_fu_units.map(f =>
      (f.io.resp.valid, f.io.resp.bits.data_securitytag)))

    io.data_check_unpass := PriorityMux(iresp_fu_units.map(f =>
      (f.io.resp.valid,f.io.data_check_unpass)))                                                                                               //data_check_unpass
    ...
    if (hasAlu) {
      ...
      io.iresp.bits.uop.csr_addr_securitytag  := alu.io.resp.bits.uop.imm_packed_securitytag

      io.iresp.bits.uop.ctrl.csr_cmd := alu.io.resp.bits.uop.ctrl.csr_cmd
    }

  dontTouch( io.iresp.bits.uop.csr_addr_securitytag)           //tag
    ...
    hasFpiu = hasFpiu) with tile.HasFPUParameters
{
  ...
  if (hasFpu) {
    ...
    fpu.io.req.bits.uop_securitytag  := 0.U(2.W)            //default  
    fpu.io.req.bits.rs1_data_securitytag  := 0.U(2.W)
    fpu.io.req.bits.rs2_data_securitytag  := 0.U(2.W)
    fpu.io.req.bits.rs3_data_securitytag  := 0.U(2.W)
  ...
  if (hasFdiv) {
    ...
    fdivsqrt.io.req.bits.rs3_data_securitytag  := 0.U(2.W)     //default
    fdivsqrt.io.req.bits.uop_securitytag       := 0.U(2.W)
    fdivsqrt.io.req.bits.rs1_data_securitytag  := 0.U(2.W)
    fdivsqrt.io.req.bits.rs2_data_securitytag  := 0.U(2.W) 
  ...
  io.fresp.bits.data_securitytag:= PriorityMux(fu_units.map(f => (f.io.resp.valid, f.io.resp.bits.data_securitytag)))

  io.fresp.bits.fflags := Mux(fpu_resp_val, fpu_resp_fflags, fdiv_resp_fflags)

  // Outputs (Write Port #1) -- FpToInt Queuing Unit -----------------------  浮点转换int单元
  ...
  if (hasFpiu) {
   ...
   queue.io.enq.bits.data_securitytag   := fpu.io.resp.bits.data_securitytag
    ...
    fp_sdq.io.enq.bits.data_securitytag  := io.req.bits.rs2_data_securitytag
