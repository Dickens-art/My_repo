  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  val io = new freechips.rocketchip.tile.CoreBundle
  {
    ...
    val interrupts = Input(new freechips.rocketchip.tile.CoreInterrupts())             //中断控制
    val ifu = new boom.ifu.BoomFrontendIO_Security                                                   //ifu已经加入了secutitytag部分
    val ptw = Flipped(new freechips.rocketchip.rocket.DatapathPTWIO())           //
    val rocc = Flipped(new freechips.rocketchip.tile.RoCCCoreIO())
    val lsu = Flipped(new boom.lsu.LSUCoreIO_Security)                                            //lsu也加入了securitytag部分
  ...
  val exe_units = new boom.exu.ExecutionUnits(fpu=false)                                 //执行单元阵列
  ...
  val decode_units     = for (w <- 0 until decodeWidth) yield { val d = Module(new DecodeUnit_Security); d }
  ...
  val issue_units      = Seq(mem_iss_unit, int_iss_unit)                                         //发射单元
  val dispatcher       = Module(new BasicDispatcher)

  val iregfile         = Module(new RegisterFileSynthesizable(                                 //
                             numIntPhysRegs,                                                                                     //整型物理寄存器数量
                             numIrfReadPorts,
                             numIrfWritePorts,
                             xLen,
  ...
  val ll_wbarb         = Module(new Arbiter(new ExeUnitResp(xLen), 1 +               //多路选择器（仲裁器），选择
  ...
  val pred_wakeup  = Wire(Valid(new ExeUnitResp_securitytag(1)))
  ...
  val dec_uops_securitytag = Wire(Vec(coreWidth, UInt(2.W)))                  //tag
  ...
  // 来自于每个alu的单个分支解析
  //（和tag无关，应该不用管）对的
  // 分支预测是只预测pc（还得预测这个pc的tag吧），然后pc送到icache重新取指令吗
 // 上述说法是错误的，因为实际是当前分支指令预测后取到下一条指令的tag，然后进行安全检查，如果通过，则更新pc_tag，即pc_tag不需要预测
  ...
  for (i <- 0 until memWidth) {
    mem_units(i).io.lsu_io <> io.lsu.exe(i)                    //addr及addr_securitytag都在这里
  }
...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  val debug_tsrc_reg = RegInit(0.U(xLen.W))
  ...
  // Breakpoint info
  //似乎是控制中断异常的
  io.ifu.status  := csr.io.status          //status是mstatus，即控制状态寄存器
  io.ifu.bp      := csr.io.bp                   //BP是分支预测
  io.ifu.mcontext := csr.io.mcontext    //和机器模式有关的？
  io.ifu.scontext := csr.io.scontext        //和监管者模式有关的？
  ...
  //上述情况（逻辑表达式，原子指令或debug时候）出现一种（reduce整个取指宽度），即flush icache（以确保该指令是单独执行的？）
  //是否需要加入tag检测？（感觉不用，tag检测应该是直接报错了）
  ...
  when (RegNext(rob.io.flush.valid)) {              //rob中冲刷提交
    ...
    when (FlushTypes.useCsrEvec(flush_typ)) {            //中断异常时候的pc重定向（需要csr）
      io.ifu.redirect_pc  := Mux(flush_typ === FlushTypes.eret,
                                 RegNext(RegNext(csr.io.evec)),
                                 csr.io.evec)
    } .otherwise {
      val flush_pc = (AlignPCToBoundary(io.ifu.get_pc(0).pc, icBlockBytes)          //出现异常的指令
                      + RegNext(rob.io.flush.bits.pc_lob)
                      - Mux(RegNext(rob.io.flush.bits.edge_inst), 2.U, 0.U))
      val flush_pc_next = flush_pc + Mux(RegNext(rob.io.flush.bits.is_rvc), 2.U, 4.U)     //出现异常的下一条指令
      io.ifu.redirect_pc := Mux(FlushTypes.useSamePC(flush_typ),                            //选择重定向位置（出现异常指令还是下一条指令）（不需要csr）
                                flush_pc, flush_pc_next)

    }
    io.ifu.redirect_ftq_idx := RegNext(rob.io.flush.bits.ftq_idx)                                   //重定向后，根据idx重置重定向的ftq条目
  } .elsewhen (brupdate.b2.mispredict && !RegNext(rob.io.flush.valid)) {            //当错误预测，冲刷流水线
    val block_pc = AlignPCToBoundary(io.ifu.get_pc(1).pc, icBlockBytes)            //fetch packet的起始pc
  ...
  val dec_finished_mask = RegInit(0.U(coreWidth.W))     //解码完成mask，指示哪一条

  //-------------------------------------------------------------
  // Pull out instructions and send to the Decoders

  io.ifu.fetchpacket.ready := dec_ready                   //ifu和decode的握手
  val dec_fbundle = io.ifu.fetchpacket.bits             //ifu将fetch到的数据传递给decode
  ...
  for (w <- 0 until coreWidth) {
    dec_valids(w)                      := io.ifu.fetchpacket.valid && dec_fbundle.uops(w).valid &&
                                          !dec_finished_mask(w)
    decode_units(w).io.enq.uop         := dec_fbundle.uops(w).bits                //来自fb的uop

    decode_units(w).io.enq_securitytag      :=  dec_fbundle.uops_securitytag(w)                //tag

    decode_units(w).io.status          := csr.io.status                                                //csr status内容
    decode_units(w).io.csr_decode      <> csr.io.decode(w)                              //csr的decode内容
    decode_units(w).io.interrupt       := csr.io.interrupt                                       //中断
    decode_units(w).io.interrupt_cause := csr.io.interrupt_cause

    dec_uops(w) := decode_units(w).io.deq.uop                                                     //decode发送给rename的uop
    dec_uops_securitytag(w) := decode_units(w).io.deq_securitytag                          //来自fetch buffer的securitytag
  }
  ...
  val ftq_arb = Module(new Arbiter(UInt(log2Ceil(ftqSz).W), 3))   //仲裁器
  ...
  // 前端异常
  val xcpt_idx = PriorityEncoder(dec_xcpts)          //对decode的异常信息（指示每条指令是否异常的序列）进行优先级编码，即只处理首先发生异常的指令
  xcpt_pc_req.valid    := dec_xcpts.reduce(_||_)   //decode中当前解码的这组指令出现异常
  xcpt_pc_req.bits     := dec_uops(xcpt_idx).ftq_idx
  //rob.io.xcpt_fetch_pc := RegEnable(io.ifu.get_pc.fetch_pc, dis_ready)
  rob.io.xcpt_fetch_pc := io.ifu.get_pc(0).pc          //前端的fetch异常发生的pc传入rob

  flush_pc_req.valid   := rob.io.flush.valid                  //rob提交时的冲刷连线
  ...
  //zip函数合并两个序列为一个序列，新序列每个元素为两个序列对应元素的合并
  //map函数为将一个序列的所有元素都执行某个操作
  //以下一句为例，便是生成dec_uops.exception && dec_valids 的新序列

  dec_xcpts := dec_uops zip dec_valids map {case (u,v) => u.exception && v}   //dec_exception为异常存在信号
  val dec_xcpt_stall = dec_xcpts.reduce(_||_) && !xcpt_pc_req.ready                   //暂停信号（可能是异常也可能是不ready（满了））
  // stall fetch/dcode because we ran out of branch tags
  val branch_mask_full = Wire(Vec(coreWidth, Bool()))

//下一句的map是指对0到corewidth的这个序列，都执行后边的逻辑操作
...
//scan为规约操作，以下句例，指对dec_hazards进行连续的或规约运算，一旦有一个1出现，后边全1

  val dec_stalls = dec_hazards.scanLeft(false.B) ((s,h) => s || h).takeRight(coreWidth)  //解码暂停信号
  dec_fire := (0 until coreWidth).map(w => dec_valids(w) && !dec_stalls(w))                   //交火指示信号
  ...
  when (dec_ready || io.ifu.redirect_flush) {
    ...
    dec_finished_mask := dec_fire.asUInt | dec_finished_mask    //指示整个packet的指令的解码进度
  }
  ...
  for (rename <- rename_stages) {
    ...
    rename.io.dec_uops := dec_uops                                                            //来自decode的 uop
    rename.io.dec_uops_securitytag := dec_uops_securitytag                      //来自decode单元的 inst_securitytag（decode又来自fb，都是直接传递）
  ...
  // Outputs
  dis_uops := rename_stage.io.ren2_uops                                                              //继续跟着流水线移动
  dis_uops_securitytag   := rename_stage.io.ren2_uops_securitytag           //输出到issu、rob(rob部分已验证)
   ...
   */
  for (w <- 0 until coreWidth) {                             
    val i_uop   = rename_stage.io.ren2_uops(w)                                 //根据指令类型设置uop
    ...
    // lrs1 can "pass through" to prs1. Used solely to index the csr file.                   //重命名逻辑
    ...
    dis_uops(w).prs2 := Mux(dis_uops(w).lrs2_rtype === RT_FLT, f_uop.prs2,
  ...
  val dis_prior_slot_unique = (dis_uops zip dis_valids).scanLeft(false.B) {case (s,(u,v)) => s || v && u.is_unique}         //派遣槽异常导致的停顿逻辑
  ...
  val dis_hazards = (0 until coreWidth).map(w =>            //派遣异常
                      dis_valids(w) &&
                      (  !rob.io.ready
                      || ren_stalls(w)
  ...
  val dis_stalls = dis_hazards.scanLeft(false.B) ((s,h) => s || h).takeRight(coreWidth)             //派遣停顿
  ...
  rob.io.enq_uops_securitytag  := dis_uops_securitytag              //来自rename，已验证

  rob.io.enq_partial_stall := dis_stalls.last // TODO come up with better ROB compacting scheme.
  rob.io.debug_tsrc := debug_tsrc_reg
  ...
  for (w <- 0 until coreWidth) {
    ...
    if (coreWidth == 1) {
      ...
      dis_uops(w).rob_idx := Cat(rob.io.rob_tail_idx >> log2Ceil(coreWidth).U,                //超标量
  ...
  // rename单元传来的信号
  //dis_uops里边是什么内容
  //应该加入dis_uops_securitytag
  for (w <- 0 until coreWidth) {
    dispatcher.io.ren_uops(w).valid := dis_fire(w)
    dispatcher.io.ren_uops(w).bits  := dis_uops(w)

    dispatcher.io.ren_uops_securitytag(w) := dis_uops_securitytag(w)   //加入的tag派遣
  ...
  for (i <- 0 until issueParams.size) {
    if (issueParams(i).iqType == IQT_FP.litValue) {                //根据发射的是浮点、整形、l/s，进入对应的发射队列
       fp_pipeline.io.dis_uops <> dispatcher.io.dis_uops(i)                              //浮点

       fp_pipeline.io.dis_uops_securitytag := dispatcher.io.dis_uops_securitytag(i)              //tag，目前设计还不支持浮点，只是做个接口

    } else {
       issue_units(iu_idx).io.dis_uops <> dispatcher.io.dis_uops(i)              //l/s或int，tag跟着发射（需要到alu或lsu参与data check）

       issue_units(iu_idx).io.dis_uops_securitytag := dispatcher.io.dis_uops_securitytag(i)           //tag
  ...
  for (i <- 0 until exe_units.length) {
    if (exe_units(i).writesIrf) {
      val fast_wakeup = Wire(Valid(new ExeUnitResp_securitytag(xLen)))
      val slow_wakeup = Wire(Valid(new ExeUnitResp_securitytag(xLen)))
      ...
      fast_wakeup.bits.uop_securitytag := iss_uops_securitytag(i)          //tag
      ...
      slow_wakeup.bits.uop_securitytag := resp.bits.uop.uopc_securitytag           //tag
  ...
  pred_wakeup.bits.uop_securitytag := iss_uops_securitytag(jmp_unit_idx)         //tag

  pred_wakeup.bits.fflags := DontCare
  pred_wakeup.bits.data := DontCare

  pred_wakeup.bits.data_securitytag := DontCare         //tag
  ...
  for (w <- 0 until exe_units.length) {
    ...
    if (exe_unit.readsIrf) {
      ...
      if (exe_unit.hasMem) {
        ...
        iss_uops_securitytag(iss_idx)   :=   mem_iss_unit.io.iss_uops_securitytag(mem_iss_cnt)                //tag
        ...
        iss_uops_securitytag(iss_idx)   :=   int_iss_unit.io.iss_uops_securitytag(int_iss_cnt)                         //tag
  ...
  issue_units.map(_.io.tsc_reg := debug_tsrc_reg)
  ...
  iregister_read.io.rf_read_ports <> iregfile.io.read_ports                  //发射端和regfile的互连
  ...
  iregister_read.io.iss_uops_securitytag := iss_uops_securitytag           //tag
  ...
  val wb_wdata_securitytag = csr_exe_unit.io.iresp.bits.data_securitytag             //tag

  csr.io.rw.addr        := csr_exe_unit.io.iresp.bits.uop.csr_addr
  csr.io.rw.cmd         := freechips.rocketchip.rocket.CSR.maskCmd(csr_exe_unit.io.iresp.valid, csr_rw_cmd)    //csr
  csr.io.rw.wdata       := wb_wdata

  //csr.io.rw.wdata_securitytag       :=  wb_wdata_securitytag                                                                   //tag

  // Extra I/O
  // Delay retire/exception 1 cycle
  csr.io.retire    := RegNext(PopCount(rob.io.commit.arch_valids.asUInt))
  csr.io.exception := RegNext(rob.io.com_xcpt.valid)              //csr的异常指示位，rob到csr
  ...
  csr.io.cause     := RegNext(rob.io.com_xcpt.bits.cause)                         //rob传给csr的异常信息

  csr.io.data_check_unpass  := RegNext(rob.io.com_xcpt.bits.data_check_unpass)   //check传入csr
  csr.io.inst_check_unpass  := RegNext(rob.io.com_xcpt.bits.inst_check_unpass)

  csr.io.ungated_clock := clock

  val tval_valid = csr.io.exception &&
    csr.io.cause.isOneOf(              //异常类型
  ...
  csr.io.tval := Mux(tval_valid,                               
    RegNext(encodeVirtualAddress(rob.io.com_xcpt.bits.badvaddr, rob.io.com_xcpt.bits.badvaddr)), 0.U)   //好像是badvaddr的位数调整？
...
//   "CSRFile is being written to spuriously.")
  ...
  for (w <- 0 until exe_units.length) {
    val exe_unit = exe_units(w)
    if (exe_unit.readsIrf) {
      exe_unit.io.req <> iregister_read.io.exe_reqs(iss_idx)                 //exu和regs连接
  ...
  for (w <- 0 until coreWidth) {
    ...
    io.lsu.dis_uops_security(w):= dis_uops_securitytag(w)                  //securitytag的连接，来自rename单元，完成
  }

  // tell LSU about committing loads and stores to clear entries
  io.lsu.commit                  := rob.io.commit                                            //包含tag
  ...
  var w_cnt = 1
  iregfile.io.write_ports(0) := WritePort(ll_wbarb.io.out, ipregSz, xLen, RT_FIX)                            //写端口0（似乎是负责load指令的）
  ll_wbarb.io.in(0) <> mem_resps(0)                                                                                                                 //arbiter优先级为0 > 1 > 2
  assert (ll_wbarb.io.in(0).ready) // never backpressure the memory unit.
  for (i <- 1 until memWidth) {
    iregfile.io.write_ports(w_cnt) := WritePort(mem_resps(i), ipregSz, xLen, RT_FIX)                  //其他写端口定义
    w_cnt += 1
  }

  for (i <- 0 until exe_units.length) {
    if (exe_units(i).writesIrf) {
      val wbresp = exe_units(i).io.iresp                                     //exu的iresp，包含：alu、lsu、csr、jmp
      val wbpdst = wbresp.bits.uop.pdst                                 //写入的寄存器号
      val wbdata = wbresp.bits.data                                          //写入数据

      val wbdata_securitytag = wbresp.bits.data_securitytag         //写入数据的tag
      ...
      iregfile.io.write_ports(w_cnt).bits.addr := wbpdst              //该地址为寄存器号
      wbresp.ready := true.B
      if (exe_units(i).hasCSR) {                                                                 //如果这个exu含有与csr交互
        iregfile.io.write_ports(w_cnt).bits.data := Mux(wbReadsCSR, csr.io.rw.rdata, wbdata)     //选择写入的是csr给出的，还是正常的响应

        iregfile.io.write_ports(w_cnt).bits.data_securitytag := Mux(wbReadsCSR, 0.U(2.W), wbdata_securitytag)   //tag

      } else {
        iregfile.io.write_ports(w_cnt).bits.data := wbdata

        iregfile.io.write_ports(w_cnt).bits.data_securitytag := wbdata_securitytag               //tag
  ...
  if (enableSFBOpt) {
    ...
    pregfile.io.write_ports(0).bits.data_securitytag := jmp_unit.io.iresp.bits.data_securitytag                    //tag
  ...
  rob.io.wb_resps(0).bits   <> ll_wbarb.io.out.bits                                                                                                      //这里边只包括rob_idx、pdst、valid
  rob.io.debug_wb_valids(0) := ll_wbarb.io.out.valid && ll_uop.dst_rtype =/= RT_X                                 //如果指令需要写寄存器，即 rf_wen
  rob.io.debug_wb_wdata(0)  := ll_wbarb.io.out.bits.data                                                                                    //wdata                 

  rob.io.debug_wb_wdata_securitytag(0)  := ll_wbarb.io.out.bits.data_securitytag                                 //wdata_tag
  rob.io.data_check_unpasses(0)  := false.B //ll_wbarb.io.out.bits.data_check_unpass                                           //data_check_unpass（from mem_resps(0)），似乎不是这里传入，而是下边lxcpt

  var cnt = 1
  for (i <- 1 until memWidth) {                                                                                                                                             //访存指令写回
    ...
    rob.io.debug_wb_wdata_securitytag(cnt)  := mem_resps(i).bits.data_securitytag                             //tag
    rob.io.data_check_unpasses(cnt)  := false.B //mem_resps(i).bits.data_check_unpass                                       //data_check_unpass, 似乎不是这里传入，而是下边lxcpt

    cnt += 1
  }
  var f_cnt = 0 // rob fflags port index
  for (eu <- exe_units) {
    if (eu.writesIrf)                                    //当exu发来写逻辑寄存器信号
    {
      val resp   = eu.io.iresp
      val wb_uop = resp.bits.uop
      
      val data_check_unpass = eu.io.data_check_unpass           //exu发来的data_check_unpass

      val data   = resp.bits.data

      val data_securitytag   = resp.bits.data_securitytag              //tag
      ...
      if (eu.hasCSR) {
        ...
        rob.io.debug_wb_wdata_securitytag(cnt) := Mux(wb_uop.ctrl.csr_cmd =/= freechips.rocketchip.rocket.CSR.N,
          0.U(2.W),
          data_securitytag)               //tag

        rob.io.data_check_unpasses(cnt) :=Mux(wb_uop.ctrl.csr_cmd =/= freechips.rocketchip.rocket.CSR.N,
          false.B,
          data_check_unpass)         //data_check_unpass

      } else {
        rob.io.debug_wb_wdata(cnt) := data

        rob.io.debug_wb_wdata_securitytag(cnt) := data_securitytag      //tag
        rob.io.data_check_unpasses(cnt) := data_check_unpass                 //data_check_unpass
      }
      cnt += 1
    }
  }
  ...
  if (usingFPU) {                   //浮点的，不太用管
    for ((wdata, wakeup) <- fp_pipeline.io.debug_wb_wdata zip fp_pipeline.io.wakeups) {
      ...
      rob.io.debug_wb_wdata_securitytag(cnt) := 0.U(2.W)    //tag,注意里边的逻辑没有做，因为浮点不考虑

      rob.io.data_check_unpasses(cnt) := false.B                        //注意里边的逻辑没有做，因为浮点不考虑
  ...
  rob.io.lxcpt          <> io.lsu.lxcpt                                                      //lsu的异常信息输入rob

  //rob.io.lxcpt_data_check_unpass   := io.lsu.lxcpt_data_check_unpass         //lsu判断的那部分data_check(dmem.dcache_security_check_exception)
  ...
  if (usingFPU) {
    fp_pipeline.io.debug_tsrc_reg := debug_tsrc_reg
  }
    ...
    for (w <- 0 until coreWidth) {
      ...
      def printf_inst(uop: MicroOp) = {
        when (uop.is_rvc) {
          ...
          //printf("(0x%x)", uop.debug_inst_securitytag)        //tag
  ...
  if (usingTrace) {             //trace模块，调试用
    for (w <- 0 until coreWidth) {
      ...
      val iaddr_securitytag = rob.io.commit.uops_securitytag(w)               //tag，不确定

      io.trace(w).iaddr      := Sext(iaddr, xLen)

    //  io.trace(w).iaddr_securitytag      := iaddr_securitytag         //tag
    ...
    //  io.trace(w).wdata_securitytag.map { _ := RegNext( rob.io.commit.debug_wdata(w)) }     //tag               
