 ...
 */
class RRdCtrlSigs(implicit p: Parameters) extends BoomBundle              //读寄存器控制信号
{
  val br_type          = UInt(BR_N.getWidth.W)                                                        //转移类型
  val use_alupipe      = Bool()                                                                                       //是否使用alu
  val use_muldivpipe   = Bool()                                                                                  //是否使用乘除法
  val use_mempipe      = Bool()                                                                                   //是否使用访存
  val op_fcn      = Bits(SZ_ALU_FN.W)                                                                       //alu类型
  val fcn_dw      = Bool()                                                                                                  //数据位宽选择，默认为64
  val op1_sel     = UInt(OP1_X.getWidth.W)                                                            //rs1选择，在reg存的值、0、pc之间选择
  val op2_sel     = UInt(OP2_X.getWidth.W)                                                            //rs2选择，在reg、imm、0、2/4、csr的imm之间选择
  val imm_sel     = UInt(IS_X.getWidth.W)                                                               //imm扩展选择，根据不同指令类型把不同位数的imm扩展到64bit
  val rf_wen      = Bool()                                                                                                   //写reg使能信号
  val csr_cmd     = Bits(CSR.SZ.W)                                                                               //csr的类型

  def decode(uopc: UInt, table: Iterable[(BitPat, List[BitPat])]) = {               //decode单元
 ...
 */
abstract trait RRdDecodeConstants                        //读寄存器解码常数抽象定义（由一个键值对的表组成，输入指令的opcode，输出指令对应的控制信号）
{
  val default: List[BitPat] =
               List[BitPat](BR_N , Y, N, N, FN_ADD , DW_X  , OP1_X   , OP2_X   , IS_X, REN_0, CSR.N)         //bitpat是一种用作二进制串匹配的类型
  val table: Array[(BitPat, List[BitPat])]                  //table为查找表
}
...
//查找表，表项是一系列键-值对，键-值对在 scala 中使用 -> 表示，左边是键，右边是值。用于表示某条指令对应的控制信号的具体取值。

object AluRRdDecode extends RRdDecodeConstants             //alu读寄存器常数
{
  val table: Array[(BitPat, List[BitPat])] =                                       //此表中，键 为 opcode，即指令低7位，值 为 该指令的控制信号                                     
 ...
 */
class RegisterReadDecode(supportedUnits: SupportedFuncUnits)(implicit p: Parameters) extends BoomModule                //寄存器读操作解码模块
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  val io = IO(new BoomBundle {                                                   //输入为发射的有效信号和微码
    val iss_valid = Input(Bool())
    val iss_uop   = Input(new MicroOp())                                   //

    val iss_uop_securitytag = Input(UInt(2.W))                      //此处应该不需要变化，因为读寄存器不涉及tag部分,稳妥起见，后边看连接时候再改

    val rrd_valid = Output(Bool())                                                 //输出为读到的微码和有效信号
    val rrd_uop   = Output(new MicroOp())

    val rrd_uop_securitytag  = Output(UInt(2.W))                  //稳妥起见

  })

  // Issued Instruction
  val rrd_valid = io.iss_valid                                                               //发射给寄存器堆的信号
  io.rrd_uop   := io.iss_uop

  io.rrd_uop_securitytag := io.iss_uop_securitytag               //tag的传递

  var dec_table = AluRRdDecode.table                                         //表
  if (supportedUnits.jmp) dec_table ++= JmpRRdDecode.table            //根据输入的支持模块类型，生成不同类型的控制信号
  ...
  io.rrd_uop.ctrl.br_type := rrd_cs.br_type                                                                                      //rrd.uop的生成，包含转移类型，rs的选择，位宽，指令类型
  ...
  when (io.rrd_uop.uopc === uopAMO_AG || (io.rrd_uop.uopc === uopLD && io.rrd_uop.mem_cmd === M_XLR)) {
    io.rrd_uop.imm_packed := 0.U                     //当为以上 指令时，不需要立即数                         
  }

  val raddr1 = io.rrd_uop.prs1 // although renamed, it'll stay 0 if lrs1 = 0       
  val csr_ren = (rrd_cs.csr_cmd === CSR.S || rrd_cs.csr_cmd === CSR.C) && raddr1 === 0.U            //csr读使能信号
  io.rrd_uop.ctrl.csr_cmd := Mux(csr_ren, CSR.R, rrd_cs.csr_cmd)                                                             //选择csr的类型
