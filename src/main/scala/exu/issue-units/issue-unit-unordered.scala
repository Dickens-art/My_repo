  ...
  extends IssueUnit(params.numEntries, params.issueWidth, numWakeupPorts, params.iqType, params.dispatchWidth)
{
  ...
  for (i <- 0 until numIssueSlots) {
    ...
    issue_slots(i).in_uop_securitytag  := Mux1H(entry_wen_oh(i), dis_uops_securitytag)             //tag
  ...
  for (w <- 0 until issueWidth) {
    ...
    io.iss_uops_securitytag(w)   := 0.U(2.W)                   
    //值得商榷的是，nop指令的securitytag给多少？这会导致安全域的切换吗？（逻辑上应该是不会的，因为nop指令不实现任何操作）
  ...
  for (w <- 0 until issueWidth) {
    ...
    for (i <- 0 until numIssueSlots) {
      ...
      when (hi_request_not_satisfied(i) && can_allocate && !port_issued) {
        ...
        io.iss_uops_securitytag(w)       := issue_slots(i).uop_securitytag
