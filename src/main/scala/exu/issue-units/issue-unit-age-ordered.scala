  ...
  extends IssueUnit(params.numEntries, params.issueWidth, numWakeupPorts, params.iqType, params.dispatchWidth)
{
  ...
  val uops = issue_slots.map(s=>s.out_uop) ++ dis_uops.map(s=>s)
   ...
   val uops_securitytag = issue_slots.map(s=>s.out_uop_securitytag) ++ dis_uops_securitytag.map(s=>s)      //tag

  for (i <- 0 until numIssueSlots) {
    issue_slots(i).in_uop.valid := false.B
    issue_slots(i).in_uop.bits  := uops(i+1)

    issue_slots(i).in_uop_securitytag  := uops_securitytag(i+1)                     //tag
    ...
    for (j <- 1 to maxShift by 1) {
      when (shamts_oh(i+j) === (1 << (j-1)).U) {
        ...
        issue_slots(i).in_uop_securitytag  := uops_securitytag(i+j)                  //tag
  ...
  for (w <- 0 until issueWidth) {
    ...
    io.iss_uops_securitytag(w)   := 0.U(2.W)          //tag
  ...
  for (i <- 0 until numIssueSlots) {
    ...
    for (w <- 0 until issueWidth) {
      ...
      when (requests(i) && !uop_issued && can_allocate && !port_issued(w)) {
        ...
        io.iss_uops_securitytag(w) := issue_slots(i).uop_securitytag                  //tag
