...
class NBDTLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends BoomModule()(p) {
  ...
  val io = IO(new Bundle {
    val req = Flipped(Vec(memWidth, Decoupled(new TLBReq_Security(lgMaxSize))))     //
  ...
  val ae_st_array = widthMap(w =>
    ...
    Mux(cmd_write_perms(w)   , ae_array(w) | (~Fill(14, io.req(w).bits.is_store_security_tag) & ~pw_array(w)), 0.U) |
  ...
  val pf_st_array = widthMap(w => Mux(cmd_write_perms(w), ~((Fill(14, io.req(w).bits.is_store_security_tag) | w_array(w)) | ptw_ae_array(w)), 0.U))
...
//Security_Tag
class TLBReq_Security(lgMaxSize: Int)(implicit p: Parameters) extends TLBReq(lgMaxSize)(p) {          //TLB改动，该信号指示的是什么？
  val is_store_security_tag = Bool()
}
