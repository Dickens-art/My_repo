 ...
 */
class ICacheResp(val outer: ICache) extends Bundle                  //看下边带securitytag的
{
 ...
 * @param outer top level ICache class
 */
class ICacheBundle(val outer: ICache) extends BoomBundle()(outer.p)              //看下边带securitytag的
 ...
 */
@chiselName
class ICacheModule(outer: ICache) extends LazyModuleImp(outer)            //定义icache
  with HasBoomFrontendParameters
{
  val enableICacheDelay = tileParams.core.asInstanceOf[BoomCoreParams].enableICacheDelay       //延迟使能
  val io = IO(new ICacheBundle_Security(outer))
  ...
  require (tl_out.d.bits.data.getWidth == wordBits+wordBits/16 || (2*tl_out.d.bits.data.getWidth == wordBits && nBanks == 2))
  ...
  val s0_vaddr = io.req.bits.addr                        //s0_vpc
  ...
  val s2_dout   = Wire(Vec(nWays, UInt((wordBits+wordBits/16).W)))
  ...
  val dataArrays = if (nBanks == 1) {
    ...
    (0 until nWays).map { x =>
      DescribedSRAM(
        name = s"dataArrayB0Way_${x}",                                          //bank0
        desc = "ICache Data Array",
        size = nSets * refillCycles,
        data = UInt(((wordBits+wordBits/16)/nBanks).W)
      )} ++
    (0 until nWays).map { x =>
      DescribedSRAM(
        name = s"dataArrayB1Way_${x}",                                          //bank1
        desc = "ICache Data Array",
        size = nSets * refillCycles,
        data = UInt(((wordBits+wordBits/16)/nBanks).W)
      )}
  ...
  if (nBanks == 1) {
    ...
    for ((dataArray, i) <- dataArrays.map(_._1) zipWithIndex) {
      def row(addr: UInt) = addr(untagBits-1, blockOffBits-log2Ceil(refillCycles))         //行索引？
      val s0_ren = s0_valid
    ...
    // Bank0 row's id wraps around if Bank1 is the starting bank.                         //两个bank的行索引生成函数
    def b0Row(addr: UInt) =
      if (refillsToOneBank) {
    ...
    for (i <- 0 until nWays) {
      val s0_ren = s0_valid                                                                                              //读使能
      val wen = (refill_one_beat && !invalidated)&& repl_way === i.U       //写使能
      ...
      if (refillsToOneBank) {
        ...
        when (wen) {                                                                         //icahce的写入
          val data = tl_out.d.bits.data                                         //mem/dcache 传给icache的？
          // val data = Wire(UInt(136.W))
          // val tag = WireInit((0xff).U(8.W))
          // dontTouch(tag)
          // data := tag ## tl_out.d.bits.data(127, 0)
          dataArraysB0(i).write(mem_idx0, Cat(data(wordBits+wordBits/32-1, wordBits), data(wordBits/2-1, 0)))               //格式为 cat（tag，data）
          dataArraysB1(i).write(mem_idx1, Cat(data(wordBits+wordBits/16-1, wordBits+wordBits/32), data(wordBits-1, wordBits/2)))
        }
      }
      if (enableICacheDelay) {
        s2_dout(i) := Cat(dataArraysB1(i).read(RegNext(mem_idx1), RegNext(!wen && s0_ren)),                //读出两个bank中的数据
  ...
  io.perf.acquire := tl_out.a.fire        //是否采用交火fire传输
 ...
 * @param outer top level ICache class
 */
class ICacheBundle_Security(val outer: ICache) extends BoomBundle()(outer.p)
  with HasBoomFrontendParameters
{
  val req = Flipped(Decoupled(new ICacheReq))            //icache的输入（paddr）
  val s1_paddr = Input(UInt(paddrBits.W))                        // delayed one cycle w.r.t. req
  ...
  val resp = Valid(new ICacheResp_Security(outer))     //icache的输出，其指令及tag
  val invalidate = Input(Bool())
 ...
 * @param outer top level ICache class
 */
class ICacheResp_Security(val outer: ICache) extends Bundle          //icache的输出
{
  val data = UInt((outer.icacheParams.fetchBytes*8+outer.icacheParams.fetchBytes/2).W) 
  
   //fetchbytes/2实际上是取到的数据位宽/16，即32bits搭配2bits的securitytag
