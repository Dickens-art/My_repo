 ...
 *
 * @param nEntries # of entries in the FTQ      ftq的条目数
 */
...
/**
 * IO to provide a port for a FunctionalUnit to get the PC of an instruction.        功能单元获取指令对应pc的端口
 * And for JALRs, the PC of the next instruction.                                 
 */
class GetPCFromFtqIO(implicit p: Parameters) extends BoomBundle           
{
  val ftq_idx   = Input(UInt(log2Ceil(ftqSz).W)         )  // ftqsz = 16,序列大小？一次可以存16条指令信息

  val entry     = Output(new FTQBundle)                       //输出的条目
  val ghist     = Output(new GlobalHistory)                   //全局历史

  val pc        = Output(UInt(vaddrBitsExtended.W))   //输出的pc

  //val pc_securitytag        = Output(UInt(2.W))                       //tag

  val com_pc    = Output(UInt(vaddrBitsExtended.W))     //整个fetchpacket的起始pc                     
  
  //val com_pc_securitytag   = Output(UInt(2.W))               //tag

  // the next_pc may not be valid (stalled or still being fetched)      //next_pc 未必生效
  val next_val  = Output(Bool())                                                                       
  val next_pc   = Output(UInt(vaddrBitsExtended.W))

  //val next_pc_securitytag   = Output(UInt(2.W))                     //tag

}

/**
 * Queue to store the fetch PC and other relevant branch predictor signals that are inflight in the processor.
 * 将给后端用的PC和分支预测信息则写到Fetch Target Queue中
 * （Fetch Target Queue保存着从I-Cache取得的指令的地址以及地址对应的分支预测信息；
 * 当指令被提交后，ROB会从Fetch Target Queue中取出该指令的相应信息；
 * 当取指单元重定向时Fetch Target Queue会被更新）
 * 
...
class FetchTargetQueue(implicit p: Parameters) extends BoomModule
  ...
  with HasBoomFrontendParameters
{
  val num_entries = ftqSz                                              //条目数 ftqsz = 16
  private val idx_sz = log2Ceil(num_entries)        //条目索引

  val io = IO(new BoomBundle {
    // Enqueue one entry for every fetch cycle.
    val enq = Flipped(Decoupled(new FetchBundle()))      //输入
    ...
    //val debug_fetch_pc_securitytag = Output(Vec(coreWidth,UInt(2.W)))             //tag

    val redirect = Input(Valid(UInt(idx_sz.W)))

    val brupdate = Input(new BrUpdateInfo)

    val bpdupdate = Output(Valid(new BranchPredictionUpdate))                        //包含tag
    ...
    //val ras_update_pc_securitytag  =  Output(UInt(2.W))                                           //tag
  ...
  //val pcs_securitytag  =  Reg(Vec(num_entries, UInt(2.W)))           //tag
  ...
  //val prev_pc_securitytag   =  RegInit(0.U(2.W))                 //tag

  when (do_enq) {

    pcs(enq_ptr)           := io.enq.bits.pc                       

    //pcs_securitytag(enq_ptr)   :=  io.enq.bits.pc_securitytag         //tag
    ...
    //new_entry.ras_top_securitytag       := io.enq.bits.ras_top_securitytag           //tag
    ...
    //prev_pc    := io.enq.bits.pc_securitytag      //tag
  ...
  //val ras_update_pc_securitytag = WireInit(0.U(2.W))        //tag
  ...
  //io.ras_update_pc_securitytag  := RegNext(ras_update_pc_securitytag)         //tag
  ...
  //val bpd_repair_pc_securitytag  =  Reg(UInt(2.W))      //tag
  ...
  //val bpd_pc_securitytag  = RegNext(pcs_securitytag(bpd_idx))              //tag
  ...
  when (io.redirect.valid) {
    ...
    //bpd_repair_pc_securitytag         := bpd_pc_securitytag        //tag
  ...
  when (RegNext(do_commit_update || do_repair_update || do_mispredict_update)) {
    ...
    //io.bpdupdate.bits.pc_securitytag      := bpd_pc_securitytag               //tag
  ...
  when (io.redirect.valid) {
    ...
    //ras_update_pc_securitytag  := redirect_entry.ras_top_securitytag       //tag
    ...
    //prev_pc_securitytag  := bpd_pc_securitytag        //tag
  ...
  for (i <- 0 until 2) {
    ...
    //val next_pc_securitytag  =  Mux(next_is_enq, io.enq.bits.pc_securitytag, pcs_securitytag(next_idx))       //tag
    ...
    //io.get_ftq_pc(i).pc_securitytag        := RegNext(pcs_securitytag(idx))          //tag

    io.get_ftq_pc(i).next_pc   := RegNext(next_pc)

    //io.get_ftq_pc(i).next_pc_securitytag   := RegNext(next_pc_securitytag)        //tag
    ...
    //io.get_ftq_pc(i).com_pc_securitytag    := RegNext(pcs_securitytag(Mux(io.deq.valid, io.deq.bits, deq_ptr)))          //tag
  ...
  for (w <- 0 until coreWidth) {
    ...
    //io.debug_fetch_pc_securitytag(w) := RegNext(pcs_securitytag(io.debug_ftq_idx(w)))          //tag
