...
trait ScalarOpConstants
{
  ...
  val BSRC_SZ = 2                              //分支预测器的类型（1周期，2周期，3周期）
  ...
  // PC Select Signal               pc选择信号
  val PC_PLUS4 = 0.U(2.W)  // PC + 4
  val PC_BRJMP = 1.U(2.W)  // brjmp_target
  val PC_JALR  = 2.U(2.W)  // jump_reg_target

  // Branch Type                    转移类型
  val BR_N   = 0.U(4.W)  // Next                                                     正常下一条指令                                  
  val BR_NE  = 1.U(4.W)  // Branch on NotEqual                   不相等转移
  val BR_EQ  = 2.U(4.W)  // Branch on Equal                           相等转移
  val BR_GE  = 3.U(4.W)  // Branch on Greater/Equal          大于等于转移
  val BR_GEU = 4.U(4.W)  // Branch on Greater/Equal Unsigned       无符号数大于等于转移
  val BR_LT  = 5.U(4.W)  // Branch on Less Than                                       小于转移
  val BR_LTU = 6.U(4.W)  // Branch on Less Than Unsigned                无符号数小于转移
  val BR_J   = 7.U(4.W)  // Jump                                                                       跳转
  val BR_JR  = 8.U(4.W)  // Jump Register                                                   寄存器跳转

  // RS1 Operand Select Signal                                          rs1选择信号
  val OP1_RS1 = 0.U(2.W) // Register Source #1          reg存的值
  val OP1_ZERO= 1.U(2.W)                                               //0
  val OP1_PC  = 2.U(2.W)                                                   //pc
  val OP1_X   = BitPat("b??")                                            

  // RS2 Operand Select Signal                                                   rs2选择信号
  val OP2_RS2 = 0.U(3.W) // Register Source #2                   reg
  val OP2_IMM = 1.U(3.W) // immediate                                  imme
  val OP2_ZERO= 2.U(3.W) // constant 0                                 0
  val OP2_NEXT= 3.U(3.W) // constant 2/4 (for PC+2/4)   2/4 适用于pc的自加
  val OP2_IMMC= 4.U(3.W) // for CSR imm found in RS1  csr的zimme
  val OP2_X   = BitPat("b???")
  ...
  // Decode Stage Control Signals       解码控制信号
  val RT_FIX   = 0.U(2.W)
  val RT_FLT   = 1.U(2.W)
  ...
  // Micro-op opcodes                     opcode，指令机器码低7位
  ...
  val uopSTA  =  2.U(UOPC_SZ.W)   // store address generation  存储地址生成
  val uopSTD  =  3.U(UOPC_SZ.W)  // store data generation  存储数据生成
