...
// DOC include start: BOOM Parameters
  ...
  maxBrCount: Int = 4,
  numFetchBufferEntries: Int = 16,                        //fetch buffer 深度
  ...
  usingFPU: Boolean = false,         //securitytag change
...
trait HasBoomCoreParameters extends freechips.rocketchip.tile.HasCoreParameters
{
  ...
  require (coreWidth <= fetchWidth)   //取出来的 应该大于等于 能处理的

  //************************************
  // Data Structure Sizes
  val numRobEntries = boomParams.numRobEntries       // number of ROB entries (e.g., 32 entries for R10k)   //rob深度
  ...
  val ftqSz         = boomParams.ftq.nEntries                         // number of FTQ entries（fetch target queue）取指目标序列的宽度，16
  ...
  //rob的index位宽，即总共有corewidth个bank，每个bank深度为numrobrows（行数）
