...
class MultiplierResp(dataBits: Int, tagBits: Int) extends Bundle {
  ...
  val tag = UInt(tagBits.W)
//***********************************************在单元的输出接口处添加安全标签信号(数据标签和检查信号)**********************************************************
  val out_secutrity_tag = UInt(2.W)
  val data_check_unpass = Bool()
//***********************************************在单元的输出接口处添加安全标签信号(数据标签和检查信号)********************************************************
}
...
//***********************************************输出安全标签的结果**********************************************************
  ...
  val max_security_tag = Mux (  (req.in1_secutrity_tag > req.in2_secutrity_tag), 
                                  req.in1_secutrity_tag, req.in2_secutrity_tag)
  val data_check_unpass_temp = Mux ( req.uop_securitytag >= max_security_tag , false.B, true.B)
  io.resp.bits.data_check_unpass := Mux(state === s_done_mul || state === s_done_div, data_check_unpass_temp, false.B)
...
//***********************************************计算最小、最大标签和检查结果***************************************************************
  ...
  val max_security_tag = Mux (  (in.bits.in1_secutrity_tag > in.bits.in2_secutrity_tag),
                                  in.bits.in1_secutrity_tag, in.bits.in2_secutrity_tag)
  val data_check_unpass_temp = Mux ( in.bits.uop_securitytag >= max_security_tag , false.B, true.B)                               
//***********************************************计算最小、最大标签和检查结果***************************************************************
...
//***********************************************输出安全标签的结果和检查结果**********************************************************
  ...
  io.resp.bits.data_check_unpass := Pipe(in.valid, data_check_unpass_temp, latency-1).bits
//***********************************************输出安全标签的结果和检查结果**********************************************************
}
